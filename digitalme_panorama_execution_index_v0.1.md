...
规格依据：`digitalme_phase1_task_P1-PANORAMA_product_panorama_alpha.md`（v0.4）、`digitalme_renderer_foundation_R0_decision_and_migration_plan.md`（v0.1.2 **accepted**）、`digitalme_phase1_task_PAN-00R_three_part_alpha_reset.md`、`digitalme_phase1_task_PAN-01S_minimal_product_surface.md`、`digitalme_phase1_task_PAN-01S.1_subject_clarity_progressive_build.md`、`digitalme_phase1_task_PAN-00_strategy_spec_freeze.md`、`digitalme_phase1_task_PAN-01_product_panorama_home.md`（历史）、`digitalme_phase1_task_PAN-01R_sovereign_collaboration_loop.md`（历史）、`digitalme_product_spec_v0.2.md`（文内 **v0.6.3**）
...
>
> **v0.2.10 修订（2026-07-20，R0 修订 1）**：关闭 Codex 第一轮架构歧义。R0 = **`spec_revision_1` / `codex_review_changes_requested`**（历史；`ac8daf3`）。
>
> **v0.2.11 修订（2026-07-20，R0 决策 accepted）**：R0 → **v0.1.2 / `accepted`**（决策接受；implementation `not_started`；无实现分支）。补充 next load/ready 失败由 main 自动回退 legacy；修正 context 两处多余 `>`。**当前唯一任务：起草并冻结 R1 独立实施任务包，交 Codex 复核；复核通过前不得创建 R1 实现分支或修改源码。** PAN-02 仍 `planned` / `blocked`。PAN-01S 族 accepted 不变。
...
→ renderer foundation R1（起草并冻结独立实施任务包；Codex 复核通过前不得创建实现分支）← 当前唯一任务
...
| **当前唯一任务** | **起草并冻结 Renderer Foundation R1 独立实施任务包，交 Codex 复核。** 复核通过前**不得**创建 R1 实现分支或修改源码。R0 implementation = `not_started`。PAN-02 保持 `planned` / `blocked`。 |
| Acceptance basis（PAN-01S 族） | Owner real Electron runtime；baseline `cbde807`；date 2026-07-20 |
| 明确不得启动 | R1 实现分支（待任务包 Codex 复核）；未经批准的 PAN-02；PAN-03～PAN-06；P1-07 修复；Digital Org；公网协作；重开 PAN-01S |
...
| renderer foundation R1 | 最小 next shell + 整窗开关 + Playwright 骨架 | **起草中**（本轮同步后为独立任务包） | R0 `accepted` | Codex 复核任务包通过后方可创建实现分支 | R2 |
...
6. **当前唯一任务**：**起草并冻结 Renderer Foundation R1 独立实施任务包**，交 Codex 复核；**复核通过前不得创建 R1 实现分支或修改源码**。R0 决策已 **`accepted`**（v0.1.2）；implementation `not_started`。**PAN-02** 保持 `planned` / `blocked`（解锁见 R0 §16；不以 SQLite 为前提）。**不得**重开 PAN-01S。非阻断技术债见 §2.3。
