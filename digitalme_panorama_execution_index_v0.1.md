...
| PAN-01R | 主权协作闭环：依据×能力×授权×代表协作×结果处置 | `statically_verified` / `codex_review_changes_requested`（不标 accepted；Codex 第二轮最小收口已合入；70/70 + 20/20） | 规格已冻结并实现；Codex 第一轮 + 第二轮最小收口已合入 | Codex 再复核 + Owner 主路径验收 | PAN-02（验收后） |
