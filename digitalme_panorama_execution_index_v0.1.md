...
> **2026-07-27 基础设施补充（非本索引主线）**：**IDCOLLAB-MIN-01** 规格已完成并经 Codex 独立复核，状态为 `specified` / `codex_review_passed` / `ready_for_owner_acceptance`（见 `digitalme_phase1_task_IDCOLLAB-MIN-01_action_identity_and_authorization_v0.1.md`）。**DVL2-03** 已 `owner_runtime_accepted` / `accepted_as_implemented` / `implemented`。CRT-MVP-02 / 02.1 / 02.2 已 `owner_runtime_accepted`（决策 #108）。本文件仍为 `superseded_as_current_execution_index`。
