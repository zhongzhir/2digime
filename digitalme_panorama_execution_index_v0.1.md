...
> **2026-07-27 基础设施补充（非本索引主线）**：**IDCOLLAB-MIN-01** 已实现并经自动化验证，状态为 `implemented` / `codex_verified` / `ready_for_owner_runtime_acceptance`（分支 `codex/idcollab-min-01-action-identity`；见 `digitalme_phase1_task_IDCOLLAB-MIN-01_action_identity_and_authorization_v0.1.md`）。**不得**提前标 `owner_runtime_accepted`。**DVL2-03** 已 `owner_runtime_accepted` / `accepted_as_implemented` / `implemented`。CRT-MVP-02 / 02.1 / 02.2 已 `owner_runtime_accepted`（决策 #108）。本文件仍为 `superseded_as_current_execution_index`。
