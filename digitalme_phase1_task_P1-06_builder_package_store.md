...
状态：accepted（Codex 复核通过；Owner 临时演示资料验收通过；真实 Package 未改动）
...
实现分支：`codex/p1-06-builder-package-store-impl`
实现提交：`0e8ba88`、`81ba8f1`
...
- identity / Life / Policies 写路径仍属后续任务，不得在本任务验收后宣称已全部迁入 PackageStore。
