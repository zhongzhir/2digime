...
建议代码基线：`9dd6fa0`（PAN-01R 最终实现；含 PAN-01 scaffold）
规划分支：`codex/pan-00r-three-part-alpha-reset`（本轮仅文档；**不**创建实现分支）
代码 Owner：Cursor（实现阶段）
任务类型：Product Panorama Alpha / 极简产品表面收口（UI 与导航；无新业务能力）
规格依据：产品规格 **v0.6.2**（§2.0 极简原则、§2.0.1 相关性门、§3.1 / §3.1.1 极简 IA 与默认「我」细则）；`digitalme_phase1_task_PAN-00R_three_part_alpha_reset.md`

> **状态语义（强制）**
>
> - `owner_approved_for_implementation` **仅**表示 Owner 已批准进入 PAN-01S 规划，并同意在本任务包经 Codex 复核通过后实现；
> - **不**表示实现结果 `accepted`；
> - 本轮规划提交后**不得**创建实现分支、**不得**修改产品代码；
> - 实现完成后 Cursor 最多标 `statically_verified`；`accepted` 须待 Owner 主路径验收。
...
| 条件（主进程可信数据） | 主操作文案 | 路由 |
|---|---|---|
| 有待处理材料或未完成构建 | `继续完善` | 现有构建入口（`me-build` 等既有导航） |
| 有待确认 / 待审阅内容 | `继续确认` | 现有待审阅入口 |
| 无待办且主体可读 | `查看我的信息` | 现有数字之我详情 |
| Package 缺失 / 严重损坏 | 见 §6 | `继续构建` / `查看问题` / `打开设置` 之一 |

次操作（可选，不同时强制两个）：

- `开始工作` → 既有对话或做事入口；
- 或 `查看详情` → 数字之我详情（当主操作已是构建/审阅时）。
...
Package 缺失、身份损坏、分层损坏或隐私状态未知时：

- **不**显示虚假的「属于你」「仅本人访问」「状态正常」等结论；
- 默认页面只给出一条简短可理解的异常说明；
- 提供一个处理入口，如 `查看问题`、`继续构建` 或 `打开设置`；
...
1. 普通用户界面**撤下**「体验一次 Digital Me 如何代表我」；
2. 默认「我」页面不得进入 PAN-01R 五步体验；
3. **不删除** PAN-01R 后台代码：授权 preview/token、取消与迟到结果处理、DecisionAudit、adopt/reject、推理环境绑定、receipt、双结果隔离；
4. 不继续打磨 PAN-01R 五步普通用户页面；
5. PAN-01R 保留为：内部协作回路验证器、自动化测试对象、新 PAN-04 外部协作骨架的基础设施；
6. Alpha 普通用户界面可以完全没有 PAN-01R 入口；
7. 不要求为内部测试在生产界面保留隐藏按钮或可猜测路由；
8. 若 `test:pan-01r-owner-runtime` 依赖普通 CTA：允许改为显式 **test-only** 启动方式；test-only 入口不得在生产环境启用；不得为了旧测试保留失败产品表面。
...
| 帮助 | 解释产品理念和用户可理解的机制（含四承诺／成长路线） |
| 设置 | 资料位置、模型连接、版本／恢复／迁移、普通配置 |
| 高级／开发者 | Package 健康细节、DecisionAudit、推理环境、证据分类、协作回路验证、调试信息 |
...
| F | 只有一个清晰主操作，最多一个次操作 |
| G | 主操作依据真实状态进入既有构建、审阅或详情 |
| H | inbox 不劫持默认「我」入口 |
| I | 深链审阅仍可用 |
| J | 帮助中可找到四个承诺和成长路线 |
| K | 侧栏不常驻 Package／模型／能力／品牌四项 |
| L | Package 缺失时不伪造所有权、隐私或健康结论 |
| M | identity／分层损坏时给简短提示并 fail-closed |
| N | 未知 navTarget fail-closed |
| O | renderer 不能注入状态 |
| P | 进入页面前后 Package 字节不变 |
| Q | 不泄漏绝对路径、密钥或主体正文 |
| R | PAN-01R 后台 hermetic 测试仍通过 |
| S | 生产界面不存在可发现的 PAN-01R 普通用户入口 |
| T | test-only 内部验证入口不能在生产环境开启 |
...
- 测试必须服从 v0.6.2 / 本任务包，而不是让产品服从旧测试。
...
| 项 | 值 |
|---|---|
| 本轮（规划） | 冻结本任务包；同步索引／规格引用／log／rule |
| **唯一下一任务** | **Codex 复核本任务包后，创建 PAN-01S 实现分支并编码** |
| 实现分支建议名 | `codex/pan-01s-minimal-product-surface`（复核通过后再建） |
| PAN-02～PAN-06 | `planned` / `not_started` |

本规划提交**不**创建实现分支、**不**修改 `digitalme-app/**`。
