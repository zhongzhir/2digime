...
- 根据需要调用专业能力
- Windows x64 打包试用
- 「与兔机米」旁路的最小内容发现（接收侧选择，不是中央 Feed 产品）
...
长期方向是：每个 2digime 都可以成为属于真人的智能网络节点；选择权留在你自己的数字之我。当前 Alpha 已有 **最小** 内容闭环（中立目录 → 你自己的兔机米决定是否呈现）。**公开 Web 是默认可发现范围**；发现走成熟搜索和标准 Feed / sitemap / 页面 metadata。**没有**开放完整内容 Feed 产品、关注 / 订阅网络、交易或支付。

---

## 内容供应商

公开 Web 是兔机米默认的可发现内容范围。
内容进入网络后，不由中央平台建立用户画像并统一推荐，
