...
describe('D11-A 渲染层接线', () => {
  it('自然语言输入调用 work.converse；关键词路由已移除', async () => {
    ...
    // 降级/澄清时不触发任何执行入口
    assert.match(appJs, /if \(res\.degraded \|\| res\.needsClarification\) return;/);
  });
  ...
  it('确定性效果只经既有路径：采用走确认弹窗、暂停走既有暂停态', async () => {
    const appJs = await fs.readFile(path.join(root, 'electron/renderer/app.js'), 'utf8');
    assert.match(appJs, /res\.adoptRequested/);
    assert.match(appJs, /handleWorkTimelineAction\("confirm_adopt"/);
