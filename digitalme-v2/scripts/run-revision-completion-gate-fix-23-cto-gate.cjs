 ...
 * 复用加固后的 20A 入口；证据写入 _revision-completion-gate-fix-23-evidence。
 * 非 Git user_selected；不 git init；不 hooked/Fake executor。
 * 任一检查失败立即停止，不写 passed verdict。
 */
'use strict';
...
const EVIDENCE = path.join(ROOT, 'scripts', '_revision-completion-gate-fix-23-evidence');
...
const HISTORICAL_FAIL_ALLOWLIST = new Set([
...
for (const name of fs.readdirSync(EVIDENCE)) {
  if (name === 'COMPLETION_DIAGNOSIS.json') continue;
...
const report = {
  schemaVersion: 'revision-completion-gate-fix-23-cto/1',
  task: '2DIGIME-BUILD-01-REVISION-COMPLETION-GATE-FIX-23',
  startedAt: new Date().toISOString(),
  headBaseline: '20f9ed9',
...
function runFullSuite() {
  ...
  const newFails = failedNames.filter((n) => !HISTORICAL_FAIL_ALLOWLIST.has(n));
  return {
    exitCode: r.status,
    pass: Number((out.match(/ℹ pass (\d+)/) || [])[1] || 0),
    fail: Number((out.match(/ℹ fail (\d+)/) || [])[1] || 0),
    ...
    failedNames,
    newFails,
    historicalAllowlist: [...HISTORICAL_FAIL_ALLOWLIST],
...
async function main() {
  ...
  report.verdict = 'revision_completion_gate_fix_23_cto_passed';
  report.finishedAt = new Date().toISOString();
  fs.writeFileSync(
    path.join(EVIDENCE, 'GATE_SUMMARY.md'),
    [
      '# FIX-23 CTO Gate — PASSED',
      '',
...
main().catch((err) => {
  report.verdict = 'revision_completion_gate_fix_23_cto_failed';
