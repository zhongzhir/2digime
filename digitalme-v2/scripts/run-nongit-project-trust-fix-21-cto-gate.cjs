/**
 * 2DIGIME-BUILD-01-NONGIT-PROJECT-TRUST-FIX-21 — 重跑加固后 CTO real-main 全链。
 * 复用 20A 入口与断言；证据写入 _nongit-project-trust-fix-21-evidence。
 * 不 git init；不 hooked/Fake executor。
 */
'use strict';
...
const EVIDENCE = path.join(ROOT, 'scripts', '_nongit-project-trust-fix-21-evidence');
...
const USER_DATA = fs.mkdtempSync(path.join(os.tmpdir(), 'dm-21-ud-'));
const FIXTURE = fs.mkdtempSync(path.join(os.tmpdir(), 'dm-21-repo-'));
fs.writeFileSync(
  path.join(FIXTURE, 'package.json'),
  JSON.stringify({ name: 'dm-21-fixture', private: true }, null, 2) + '\n',
);
fs.writeFileSync(
...
const report = {
  schemaVersion: 'nongit-project-trust-fix-21-cto/1',
  task: '2DIGIME-BUILD-01-NONGIT-PROJECT-TRUST-FIX-21',
...
async function main() {
  ...
  report.verdict = 'nongit_project_trust_fix_21_cto_passed';
...
main().catch((err) => {
  report.verdict = 'nongit_project_trust_fix_21_cto_failed';
