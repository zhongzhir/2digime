 ...
 * 复用加固后的 20A 入口；证据写入 _ai-cto-contract-reliability-fix-25-evidence。
 * 非 Git user_selected；不 git init；不 hooked/Fake executor。
 * 第二轮 CTO 必须成功解析；降级模板不得 PASSED。
 * 全量与 1f759af 的 541/20/1 逐项比较；不得新增失败。
 */
'use strict';
...
const EVIDENCE = path.join(ROOT, 'scripts', '_ai-cto-contract-reliability-fix-25-evidence');
...
/** 1f759af 全量失败名；比较用。不得把本轮新增失败加入此集合。 */
const BASELINE_1F759AF_FAIL_NAMES = new Set([
...
const BASELINE_1F759AF = { pass: 541, fail: 20, skip: 1, head: '1f759af' };

fs.mkdirSync(EVIDENCE, { recursive: true });
fs.mkdirSync(SHOTS, { recursive: true });
for (const name of fs.readdirSync(EVIDENCE)) {
  if (name === 'AI_CTO_PARSE_DIAGNOSIS.json' || name === 'FIX24_ROUND2_CONTRACT_DIAGNOSIS.json') continue;
...
fs.writeFileSync(
  path.join(EVIDENCE, 'FIX24_ROUND2_CONTRACT_DIAGNOSIS.json'),
  `${JSON.stringify(
    {
      schemaVersion: 'ai-cto-parse-diagnosis/1',
      source: 'fix-24-round-2-postmortem',
      head: '1f759af',
      savedModelText: false,
      savedChainOfThought: false,
      cacheMisuse: false,
      keywordRouting: false,
      parsedNullExecutionRestored: false,
      attempts: [
        {
          attempt: 1,
          foundJson: null,
          jsonParseOk: false,
          illegalDecision: null,
          invalidEvidenceRefs: null,
          missingOrTypedWrong: [],
          textLength: null,
          finishReason: 'not_recorded',
          truncated: 'unknown_finish_reason_dropped_by_cto_chat',
          failStep: 'no_machine_readable_diagnosis_at_fix24',
          structuralCauses: [
            'cto_chat_returned_only_text_dropped_finish_reason',
            'maxTokens_2048',
            'response_format_json_object_only_no_json_schema',
            'generic_repair_prompt',
            'evidence_pack_paths_only_no_file_or_test_excerpts',
            'extractUnderstandingPaths_required_slash_missed_formatLabel.js',
            'pre_exec_locate_risks_无需修改_written_into_v2_bundle',
            'summaryExcerpt_started_with_original_goal_section',
          ],
        },
        {
          attempt: 2,
          foundJson: null,
          jsonParseOk: false,
          failStep: 'generic_repair_same_contract_still_unparseable',
          structuralCauses: ['repair_not_targeted_to_fail_step'],
        },
      ],
      gateDefect: 'fix24_gate_passed_unavailableReview_five_point_template',
      notModelFluke: true,
    },
    null,
    2,
...
const report = {
  schemaVersion: 'ai-cto-contract-reliability-fix-25-cto/1',
  task: '2DIGIME-BUILD-01-AI-CTO-CONTRACT-RELIABILITY-FIX-25',
  startedAt: new Date().toISOString(),
  headBaseline: '1f759af',
...
function runNodeTests() {
  const tests = [
    ...
    'dist/execution/tests/ai-cto-contract-reliability-fix-25.test.js',
    'dist/execution/tests/ai-cto-review-14.test.js',
...
function runFullSuite() {
  ...
  const newFails = uniqueFails.filter((n) => !BASELINE_1F759AF_FAIL_NAMES.has(n));
  const fixedVsBaseline = [...BASELINE_1F759AF_FAIL_NAMES].filter((n) => !uniqueFails.includes(n));
  return {
    exitCode: r.status,
    pass: Number((out.match(/ℹ pass (\d+)/) || [])[1] || 0),
    fail: Number((out.match(/ℹ fail (\d+)/) || [])[1] || 0),
...
async function main() {
  ...
  report.verdict = 'ai_cto_contract_reliability_fix_25_cto_passed';
  report.finishedAt = new Date().toISOString();
  fs.writeFileSync(
    path.join(EVIDENCE, 'GATE_SUMMARY.md'),
    [
      '# FIX-25 CTO Gate — PASSED',
      '',
...
main().catch((err) => {
  report.verdict = 'ai_cto_contract_reliability_fix_25_cto_failed';
