 ...
 * 定位并防止两类历史错误:
 * 1) 把 `console.log(process.versions.electron)` 当成应用路径 → 出现 D:\console.log(...)
 * 2) 拼接含空格路径 `D:\Projects\Digital Me` → 截断成 D:\Projects\Digital
 */
    ...
    JSON.stringify(
      {
        ...
        electronPath,
        notes: [
          "历史错误 D:\\\\console.log(process.versions.electron) 来自把 -e 表达式当应用路径",
          "历史错误 D:\\\\Projects\\\\Digital 来自未引号的空格路径拼接",
