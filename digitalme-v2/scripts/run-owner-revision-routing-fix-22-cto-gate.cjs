/**
 * 2DIGIME-BUILD-01-OWNER-REVISION-ROUTING-FIX-22 — 重跑加固后 CTO real-main 全链。
 * 复用 20A 入口；证据写入 _owner-revision-routing-fix-22-evidence。
...
const EVIDENCE = path.join(ROOT, 'scripts', '_owner-revision-routing-fix-22-evidence');
...
// 保留 ROUTING_DIAGNOSIS.json：只清 shots / 运行产物，不整目录 rm
for (const name of fs.readdirSync(EVIDENCE)) {
  if (name === 'ROUTING_DIAGNOSIS.json') continue;
  const p = path.join(EVIDENCE, name);
  fs.rmSync(p, { recursive: true, force: true });
}
fs.mkdirSync(SHOTS, { recursive: true });

const USER_DATA = fs.mkdtempSync(path.join(os.tmpdir(), 'dm-22-ud-'));
const FIXTURE = fs.mkdtempSync(path.join(os.tmpdir(), 'dm-22-repo-'));
fs.writeFileSync(
  path.join(FIXTURE, 'package.json'),
  JSON.stringify({ name: 'dm-22-fixture', private: true }, null, 2) + '\n',
);
fs.writeFileSync(
...
const report = {
  schemaVersion: 'owner-revision-routing-fix-22-cto/1',
  task: '2DIGIME-BUILD-01-OWNER-REVISION-ROUTING-FIX-22',
...
async function main() {
  ...
  report.verdict = 'owner_revision_routing_fix_22_cto_passed';
...
main().catch((err) => {
  report.verdict = 'owner_revision_routing_fix_22_cto_failed';
