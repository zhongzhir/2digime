...
import {
  checkDisclosure,
  createInMemorySubjectNetwork,
  formatPublicCardsForModel,
  listExchanges,
  PUBLIC_CARD_SCHEMA_VERSION,
  writePublicCard,
} from '../../subject-collab';
import type { PublicSubjectCard } from '../../subject-collab';

async function tempDir(prefix: string): Promise<string> {
  return fs.mkdtemp(path.join(os.tmpdir(), `dm-dual-${prefix}-`));
}
...
test('披露闸门拒绝完整 Digital Self 与敏感身份', () => {
  const self = '- 我是张三\n- 我在做化工厂改造\n- 私人住址是西湖区某某路88号'.repeat(4);
  const blocked = checkDisclosure({
    disclosure: self,
    selfContext: self,
    threadText: 'user: 帮我分析这个项目',
  });
  assert.equal(blocked.ok, false);
  if (!blocked.ok) assert.match(blocked.reason, /完整 Digital Self/);
  const secret = checkDisclosure({
    disclosure: '护照号码 E12345678，用于这次合作。',
    selfContext: '短',
    threadText: '短',
  });
...
test('A 从「与 2digime」自主选择 B，B 独立贡献后回到原 Thread', async () => {
  const root = await tempDir('ok');
  const net = createInMemorySubjectNetwork();
  let bRuns = 0;
  ...
  const aRt = createDigitalMeRuntime({
    ...
    talkChat: scriptedChat([
      async ({ messages, tools }) => {
        ...
        assert.equal(
          tools?.some((item) => item.function.name === 'delegate'),
          false,
        );
        assert.match(sys, /当前可发现的其他主体/);
        assert.equal(sys.includes('西湖区某某路88号'), false);
        assert.equal(sys.includes('if test'), false);
  ...
  const createdA = await aBus.invoke('subject.createPackage', {
    displayName: '主体甲',
    targetDir: aDir,
  });
  ...
  const cardsForA = await net.listCards(createdA.subjectId);
  const cardText = formatPublicCardsForModel(cardsForA);
  assert.match(cardText, /工艺安全/);
  assert.match(cardText, /现代诗/);
  assert.equal(cardText.includes('西湖区某某路88号'), false);
  assert.equal(cardText.includes('不得外发'), false);
  ...
  assert.equal(blob.includes('consult_subject'), false);
  assert.equal(blob.includes('A2A'), false);
  ...
  assert.ok(bEx.some((item) => item.kind === 'response_sent' && item.usedOwnCapability));
  const disclosed = aEx.find((item) => item.kind === 'request_sent')?.body || '';
  ...
  assert.equal(
    JSON.stringify([...aEx, ...bEx]).includes('NEGOTIATING') ||
      JSON.stringify([...aEx, ...bEx]).includes('CollabUserStatus'),
    false,
  );

  const rec = JSON.parse(await fs.readFile(talkThreadFilePath(aDir), 'utf8')) as {
    executions: unknown[];
    turns: Array<{ exchangeIds?: string[] }>;
  };
  assert.equal(rec.executions.length, 0);
  assert.ok(rec.turns.some((turn) => (turn.exchangeIds || []).length > 0));

  const bRunsDir = path.join(bDir, 'intelligence', 'runs');
  const aRunsDir = path.join(aDir, 'intelligence', 'runs');
  const bRunNames = await fs.readdir(bRunsDir);
  assert.ok(bRunNames.length >= 1);
  await assert.rejects(() => fs.readdir(aRunsDir));
...
test('B 拒绝后 A 不得把失败改写成合作成功，且不新增协作状态机', async () => {
  ...
  const aEx = await listExchanges(aDir);
  assert.ok(aEx.some((item) => item.kind === 'response_received' && item.peerDecision === 'decline'));
  assert.equal(
    aEx.some((item) => /INVITED|ACCEPTED|NEGOTIATING|ASSIGNED/.test(item.kind)),
    false,
  );
  await aRt.stop();
  await bRt.stop();
