...
export interface CommandMap {
  ...
  'subject.getOverview': {
    ...
    output: {
       ...
       * 仅含具体可引用的本人事实（含具体值 detail；排除维度名、泛化摘要、临时/外部/候选/失效/内部过程）。
       */
      userVisibleFacts?: Array<{
        eventId: string;
        kind: import('../subject-core/derive-all').ActiveSubjectItem['kind'];
        title: string;
        detail: string;
        text: string;
      }>;
      ...
      growth?: {
        ...
        tracks: {
          represent: { name: string; status: string; statusKey: 'pending' | 'started' | 'stable' };
          work: { name: string; status: string; statusKey: 'pending' | 'started' | 'stable' };
          collab: { name: string; status: string; statusKey: 'pending' | 'started' | 'stable' };
        };
        ...
        guidedQuestion?: { dimensionKey: string; text: string } | null;
        cockpit?: {
          knownCount: number;
          knownPreview: Array<{ key: string; name: string; summary: string }>;
          knownHasMore: boolean;
          gaps: Array<{ dimensionKey: string; name: string; question: string }>;
          materials: {
            total: number;
