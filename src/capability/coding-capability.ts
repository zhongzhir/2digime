...
/** 多 ready 能力时的默认选择。专用执行器（非模型兜底）优先；模型兜底（api）仅在没有 ready 专用执行器时选择。 */
export function selectPreferredCodingCapability(
  ...
  // 专用执行器 ready 时，不得选模型运输（§2.4）。模型兜底用 capabilityId 区分。
  const dedicated = readyAuto.filter(
    (s) => s.capabilityId !== EXTERNAL_EXECUTOR_MODEL_API_CAPABILITY_ID,
  );
  const pool = dedicated.length > 0 ? dedicated : readyAuto;
  const preferredId = String(prefs?.defaultCapabilityId || '').trim();
  if (preferredId) {
    const hit = pool.find((s) => s.capabilityId === preferredId);
    if (hit) return hit;
  }
  const scored = [...pool].sort((a, b) => scoreCodingCapability(b) - scoreCodingCapability(a));
...
export function buildCodingOnboardingPayload(
  ...
  if (statuses.some((s) => s.availability === 'needs_login')) {
    ...
    message =
      '尚未检测到可用的代码执行工具。若你已在设置中连接模型，可由它完成本次小改动；否则请先在设置中连接模型后再试。';
  }
  return {
