...
export interface CapabilitySelectResult {
  ...
  /** 用户可行动说明（能力不可用或无法匹配时）。 */
  actionable?: string;
  /** MULTI-AGENT-ROUTE-01：代码修改路由详情（审计）。 */
  codingAgentRoute?: CodingAgentRouteResult;
}
...
export class CapabilityRegistry {
  ...
  selectForNeed(need: CapabilityNeed): CapabilitySelectResult {
    ...
    if (intent === 'modify_code' || family === CODE_CHANGE_ARTIFACT_TYPE) {
      ...
      // MULTI-AGENT-ROUTE-01：显式路由，而非「Codex 否则任意 code-change」。
      // 避免把「检测到但不能自动调用」的 unsupported 桌面工具当成执行器。
      const route = routeCodingAgent({
        adapters: [...this.adapters.values()],
        ...(need.explicitCapabilityId
          ? { explicitCapabilityId: need.explicitCapabilityId }
          : {}),
      });
      if (route.adapter) {
        return {
          ...
          reason:
            route.reason === 'primary'
              ? 'primary'
              : route.reason === 'model_api'
                ? 'model_api'
                : 'explicit',
          codingAgentRoute: route,
        };
      }
      return {
        reason: 'none',
        actionable:
          route.actionable ||
          '当前无法修改项目文件：请先在设置中连接代码执行组件后再试。不会改用普通写作冒充代码修改。',
        ...(route.capabilityId ? { codingAgentRoute: route } : {}),
      };
    }
