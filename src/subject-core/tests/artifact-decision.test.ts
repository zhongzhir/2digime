...
test('distill acceptance includes evidence and decision tag; no second confirmation', () => {
  ...
  const doing = events.find((e) => e.payload.tags?.includes('doing_experience'));
  assert.ok(doing);
  assert.equal(doing!.payload.title, '近期完成的工作');
  assert.match(doing!.payload.detail, /简洁/);
...
test('对话近况「我是在修改项目」不是身份，进入最近在做的事', () => {
  const events = distillCandidatesFromText({
    subjectId: 's',
    text: '我是在修改两个项目，一个是显示名项目，一个是库存项目。',
    sourceKind: 'conversation',
  });
  assert.equal(events.some((e) => e.type === 'identity_clarified'), false);
  const doing = events.find((e) => e.payload.tags?.includes('doing_experience'));
  assert.ok(doing);
  assert.equal(doing!.payload.title, '最近在做的事');
  assert.match(doing!.payload.detail, /显示名|库存/);
  const identity = distillCandidatesFromText({
    subjectId: 's',
    text: '我是本地优先产品负责人',
    sourceKind: 'conversation',
  });
  assert.ok(identity.some((e) => e.type === 'identity_clarified'));
});
