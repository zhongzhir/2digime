...
export class SubjectService {
  ...
  async getOverview(_input: Record<string, never> = {}): Promise<OverviewOutput> {
    ...
    return {
      ...
      userVisibleFacts: slices.userVisibleFacts,
  ...
  async captureInput(input: {
    ...
    if (input.captureKey === 'growth:current_goal') {
      return this.recordExplicitConfirmedGrowthFact({
        captureKey: input.captureKey,
        type: 'goal_updated',
        title: '当前目标',
        detail: text.slice(0, 2000),
        tags: ['growth:current_goal', 'goal'],
      });
    }
    if (input.captureKey === 'growth:basic_boundaries') {
      return this.recordExplicitConfirmedGrowthFact({
        captureKey: input.captureKey,
        type: 'boundary_updated',
        title: '基本使用边界',
        detail: text.slice(0, 2000),
        tags: ['growth:basic_boundaries', '边界'],
      });
    }
    ...
    if (isDecision && candidateEventIds.length > 0) {
      const decisionOnly = distilled.filter((e) =>
        (e.payload.tags ?? []).some(
          (t) => t === 'decision:accept' || t === 'decision:reject' || t === 'doing_experience',
        ),
      );
  ...
  private async recordExplicitConfirmedGrowthFact(input: {
    captureKey: string;
    type: 'goal_updated' | 'boundary_updated';
    title: string;
    detail: string;
    tags: string[];
  }): Promise<{
    ...
    const event: GrowthEvent = {
      ...
      payload: {
        title: input.title,
        detail: input.detail,
        tags: [...input.tags, `captureKey:${input.captureKey}`],
      },
      confidence: 'confirmed',
    };
    await this.appendGrowthEvent(event);
