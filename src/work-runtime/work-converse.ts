...
export function decideConverseEffects(input: ConverseDecisionInput): ConverseDecision {
  ...
  switch (intent) {
    ...
    case 'artifact_feedback': {
      ...
      if (isExplicitCurrentResultAcceptance(input.userText || '')) {
        decision.intent = 'final_adopt';
        decision.adoptRequested = true;
        decision.reply = CURRENT_RESULT_ACCEPTED_REPLY;
        break;
      }
    ...
    case 'confirm_start':
      ...
      if (!input.jobRunning && !input.firstTurn) {
        if (input.hasArtifact) {
          ...
          if (isExplicitCurrentResultAcceptance(userText)) {
            decision.intent = 'final_adopt';
            decision.adoptRequested = true;
            decision.reply = CURRENT_RESULT_ACCEPTED_REPLY;
            break;
          ...
          decision.needsClarification = true;
          decision.reply =
            '这一版已经做完。你是要接受当前成果并结束，还是还要改什么？请直接说。';
          break;
        }
        decision.confirmPlan = true;
        decision.startAuthorized = true;
        decision.startMode = 'new_execution';
      }
      break;
    case 'final_adopt':
      if (confidence < EXECUTION_EFFECT_CONFIDENCE_THRESHOLD) {
