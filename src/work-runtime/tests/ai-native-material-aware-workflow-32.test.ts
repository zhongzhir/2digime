...
async function writeTriageappLikeFixture(root: string) {
  await fs.mkdir(root, { recursive: true });
  await fs.writeFile(
    path.join(root, 'triageapp.html'),
    '<html><body><h1>TRIAGEAPP</h1><p>triage_items and localStorage</p><script>void 0</script></body></html>',
    'utf8',
  );
  await fs.writeFile(
  ...
  await fs.writeFile(
    path.join(root, 'TRIAGEAPP-PROJECT.md'),
    'TRIAGEAPP 是单文件内容分拣工作台，使用 localStorage 保存 triage_items。',
    'utf8',
  );
...
describe('AI-NATIVE-MATERIAL-AWARE-WORKFLOW-32', () => {
  it('HTML/CSV 进入抽取白名单并可读', async () => {
    ...
    await writeTriageappLikeFixture(root);
    const html = await extractFile(path.join(root, 'triageapp.html'));
    assert.equal(html.status, 'ok');
    assert.match(String(html.text || ''), /TRIAGEAPP/);
    ...
    assert.ok(folder.some((o) => o.sourcePath.endsWith('triageapp.html') && o.status === 'ok'));
  ...
  it('材料简报含授权路径、清单、摘录与完整性', async () => {
    ...
    assert.ok(brief.items.some((i) => i.displayName === 'triageapp.html' && i.usedChars > 0));
    ...
    assert.match(brief.promptBlock, /triageapp\.html|sample\.csv|TRIAGEAPP-PROJECT/);
  });

  it('代码项目文件夹能读到 js/json，不得标成格式暂不支持', async () => {
    const root = await tempDir('js-brief');
    await fs.mkdir(path.join(root, 'src'), { recursive: true });
    await fs.writeFile(path.join(root, 'package.json'), '{"name":"inv"}', 'utf8');
    await fs.writeFile(
      path.join(root, 'src', 'inventory.js'),
      "module.exports = { Inventory: class {} };\n",
      'utf8',
    );
    const brief = await buildConverseMaterialBrief({
      contextRefs: [{ kind: 'folder', path: root, projectOrigin: 'user_selected' }],
    ...
    const js = brief.items.find((i) => i.displayName === 'inventory.js');
    assert.ok(js);
    assert.notEqual(js!.completeness, 'unread');
    assert.match(String(js!.excerpt || ''), /Inventory/);
    assert.ok(!brief.items.some((i) => /格式暂不支持/.test(String(i.skipReason || ''))));
    assert.doesNotMatch(brief.promptBlock, /格式暂不支持/);
  });
  ...
  it('buildConverseMessages 注入材料后禁止模型装瞎的合同上下文', () => {
    const messages = buildConverseMessages({
      ...
      recentTurns: [],
      userText: '你先了解项目。',
      materialBrief: '【已授权材料】\n- 授权路径：D:\\Projects\\demo-project\n【材料清单】\n- triageapp.html（完整阅读）',
    });
    const blob = messages.map((m) => m.content).join('\n');
    assert.match(blob, /已授权材料/);
    assert.match(blob, /triageapp\.html/);
  ...
  it('多种实施表达 → Coding；只分析不改文件 → 文档；含优化实施但明确不改 → 文档', async () => {
    ...
    for (const c of cases) {
      ...
      await writeTriageappLikeFixture(project);
      let codingHits = 0;
      let docHits = 0;
  ...
  it('UI 文件夹落盘后后续 converse 仍能获得材料；确认前零 Job；确认后按模型执行族走 Coding', async () => {
      ...
      if (/提出项目优化升级方案/.test(latest)) {
        return {
          text: JSON.stringify({
            intent: 'add_goal_info',
            confidence: 0.95,
            reply: '已阅读 TRIAGEAPP 相关材料，整理了优化实施方案，请确认后开始修改项目。',
            planUpdate: [
              '目标：提出并实施 TRIAGEAPP 优化升级方案',
      ...
      if (/了解项目/.test(latest)) {
        return {
          text: JSON.stringify({
            ...
            confidence: 0.9,
            reply: '根据已读的 triageapp.html 与 CSV，这是本地分拣工作台；建议先做效率优化。',
          }),
        };
      }
      return {
    ...
    assert.match(captured[0] || '', /已授权材料|triageapp\.html|sample\.csv|TRIAGEAPP-PROJECT/i);
    ...
    assert.match(captured[1] || '', /已授权材料|triageapp\.html|TRIAGEAPP/i);
