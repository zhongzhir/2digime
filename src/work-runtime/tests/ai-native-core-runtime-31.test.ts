...
const UNIQUE_STEP = '先写入 triageapp-unique-step.txt，内容必须是 CORE-RUNTIME-31-UNIQUE';
...
describe('AI-NATIVE-CORE-RUNTIME-31', () => {
  ...
  it('执行器输入包含确认规划的独有内容，且 Job 可追溯规划版本', async () => {
    ...
    assert.match(String(jobs[0]?.confirmedPlanSnapshot?.content || ''), /triageapp-unique-step\.txt/);
    const last = captured[captured.length - 1];
    assert.ok(last);
    assert.equal(last.goal, goal);
    assert.doesNotMatch(last.goal, /triageapp-unique-step/);
    assert.ok(last.confirmedPlan);
    assert.match(last.confirmedPlan.content, /triageapp-unique-step\.txt/);
