...
describe('D11-A work.converse — AI 意图与对话中枢', () => {
  ...
  it('确定性策略层：解析失败→规划失败语义零效果；执行中/首轮不授权开始与采用', () => {
    ...
    assert.equal(revision.startAuthorized, true);
    const adoptDone = decideConverseEffects({
      parsed: { intent: 'confirm_start', confidence: 0.9, reply: '好，开始',
        executionIntentKind: 'create_document',
        expectedOutputFamily: 'document' },
      modelAvailable: true,
      hasArtifact: true,
    ...
    assert.equal(adoptDone.adoptRequested, true);
    assert.equal(adoptDone.startAuthorized, false);
    // 无成果时不得采用
