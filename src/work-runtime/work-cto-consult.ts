...
export function formatCtoUserConclusion(input: {
  ...
  return [
    ...
    `还需不需要修改：${sanitizeCtoNeedChange(input.needChange)}`,
    `需要你知道的风险：${sanitizeCtoUserFacingText(input.risks)}`,
    `建议下一步：${sanitizeCtoNextStep(input.nextStep)}`,
  ].join('\n');
}

function isCtoFragment(text: string): boolean {
  const t = String(text || '').trim();
  if (!t) return true;
  if (/^无[，,。]?$/.test(t)) return true;
  if (/^无[，,].{0,12}交付/.test(t)) return true;
  if (/可直接交付使用/.test(t) && t.length < 24) return true;
  return false;
}

function sanitizeCtoNeedChange(text: string): string {
  const t = sanitizeCtoUserFacingText(text);
  if (isCtoFragment(t)) return '不是必须再改；有意见直接说。';
  return t;
}

export function sanitizeCtoNextStep(text: string): string {
  const t = sanitizeCtoUserFacingText(text);
  if (isCtoFragment(t)) return '如需调整，直接说明即可。';
  return t;
}
