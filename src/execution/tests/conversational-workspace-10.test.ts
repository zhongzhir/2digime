...
describe('CONVERSATIONAL-WORKSPACE-10', () => {
  ...
  it('对话记录在前，结束语在后；残缺下一步不单独成条', () => {
    const turns = conv.buildWorkTimeline({
      extraTurns: [
        { role: 'digital_me', kind: 'message', text: '好的，本次任务结束。如有其它需要，请随时告诉我。' },
        { role: 'digital_me', kind: 'next_step', text: '无，可直接交付使用。' },
        { role: 'user', kind: 'message', text: '修好空格' },
        { role: 'digital_me', kind: 'message', text: '确认后开始' },
      ],
      ctoReport: '已经完成修改并检查通过。',
      userFacingNextStep: '无，可直接交付使用。',
      decisionAccepted: true,
      canAdoptSuggested: true,
      hasArtifact: true,
    });
    assert.ok(turns.length >= 2);
    assert.equal(turns[0]!.text, '修好空格');
    assert.equal(turns[turns.length - 1]!.kind, 'adopted');
    assert.ok(turns.findIndex((t) => t.kind === 'adopted') > turns.findIndex((t) => t.text === '修好空格'));
    assert.equal(turns.filter((t) => t.kind === 'adopted').length, 1);
    assert.ok(!turns.some((t) => /可直接交付使用/.test(t.text)));
    assert.ok(!turns.some((t) => t.actions?.some((a) => a.id === 'confirm_adopt')));
  });
