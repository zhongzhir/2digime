...
describe('software-development-task-ux', () => {
  ...
  it('本轮无新改动时，范围内相对 HEAD 的未提交改动仍可交付', async () => {
    const { spawnSync } = await import('node:child_process');
    const repo = await fs.mkdtemp(path.join(os.tmpdir(), 'dm-dirty-head-'));
    const evidence = await fs.mkdtemp(path.join(os.tmpdir(), 'dm-ev-dirty-'));
    spawnSync('git', ['init'], { cwd: repo, encoding: 'utf8', shell: false, windowsHide: true });
    spawnSync('git', ['config', 'user.email', 't@invalid'], {
      cwd: repo,
      encoding: 'utf8',
      shell: false,
      windowsHide: true,
    ...
    await fs.writeFile(path.join(repo, 'src.js'), 'module.exports = 1;\n', 'utf8');
    spawnSync('git', ['add', '.'], { cwd: repo, encoding: 'utf8', shell: false, windowsHide: true });
    ...
    await fs.writeFile(path.join(repo, 'src.js'), 'module.exports = 2;\n', 'utf8');
    const baseline = await captureExecutionBaseline({
      workingDirectory: repo,
      writeScope: ['.'],
      readScope: ['.'],
    ...
    const unchanged = await collectExecutionChanges({ baseline, jobEvidenceDir: evidence });
    assert.equal(unchanged.changedFiles.length, 0);
    const preexisting = await collectExecutionChanges({
      baseline,
      jobEvidenceDir: evidence,
      includeDirtyVersusHeadWhenUnchanged: true,
    });
    assert.ok(preexisting.changedFiles.includes('src.js'));
  });
