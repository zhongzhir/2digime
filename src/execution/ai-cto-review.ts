...
import { formatCtoUserConclusion, sanitizeCtoNextStep } from '../work-runtime/work-cto-consult';
...
const SYSTEM_PROMPT = [
  ...
  '若任务需要现实世界信息（规划含外部信息、或 materials 含 search-evidence）：必须判断正文是否真正回答了用户问题；关键现实判断是否能回到证据；是否只是链接或标题清单；核心问题是否被遗漏；证据明显不足时是否仍给出过强结论。仅有来源列表而无综合判断，不得 meets_plan。不得把内部知识假装成已检索到的最新事实。',
...
export function buildAiCtoEvidencePack(input: CtoReviewInput): AiCtoEvidencePack {
  ...
  return {
    ...
    ...(input.confirmedPlan && String(input.confirmedPlan.content || '').trim()
      ...
      : {}),
    ...(input.planRequirements?.length
      ? { planRequirements: input.planRequirements.slice(0, 8) }
      : {}),
    ...
    primaryAction:
      guardedDecision === 'meets_plan'
        ...
        : guardedDecision === 'needs_revision'
          ? 'confirm_continue'
          : 'need_decision',
    userFacingNextStep: sanitizeCtoNextStep(
      guardedDecision === output.decision ? output.nextAction : nextStep,
    ),
