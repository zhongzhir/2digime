...
export async function verifyExternalExecution(input: {
  ...
  /** 预注册测试（P0 基准绑定）：独立执行并逐项判定，未提供时 check 为 unverifiable。 */
  preRegisteredTests?: Array<{ command: string[]; description?: string; cwd?: string }>;
  ...
  // 2b. 实质产出（机械回复 / 空转完成检测）
  checks.push(buildSubstantiveOutputCheck(col));
  ...
  // 6a. 预注册测试（P0 基准绑定）：独立于 executor 自报，逐项执行并判定。
  checks.push(await runPreRegisteredTests(pkg, input.jobEvidenceDir, input.preRegisteredTests));
...
/** 机械回复 / 空转完成签名：无实质产出的改动不得标记为完成。 */
const MECHANICAL_DIFF_SIGNATURES =
  /AUTO_REPLY|MEKANIKAL|MECHANICAL|template[ _-]?reply|placeholder[ _-]?done|空转完成|模板回复|机械回复|no[ _-]?substantive[ _-]?change/i;

function isSubstantiveDiff(diff: string): boolean {
  ...
  const candidateLines =
    added.length > 0 ? added : extractSyntheticContentLines(text);
  if (candidateLines.length === 0) return false;
  if (candidateLines.some((l) => MECHANICAL_DIFF_SIGNATURES.test(l))) return false;
  // 仅空行 / 注释 / 分隔线不算实质产出
  const substantive = candidateLines.filter(
    (l) =>
      !/^(#|\/\/|\/\*|\*|\/\/!|<!--?|[-=*_]+$)/.test(l) &&
      l.replace(/[^\p{L}\p{N}]/gu, '').length > 0,
  );
  return substantive.length > 0;
}

/** 非 git 合成 diff：取 ``` 代码块内的实际内容行（去掉块头与分隔）。 */
function extractSyntheticContentLines(text: string): string[] {
  const out: string[] = [];
  let inFence = false;
...
function buildSubstantiveOutputCheck(
  col: CollectedExecutionChanges,
): VerificationCheckResult {
  const substantive = isSubstantiveDiff(col.unifiedDiff);
  return {
    id: 'substantive_output',
    title: '是否存在实质产出（非机械回复 / 空转）',
    verdict: substantive ? 'satisfied' : 'unsatisfied',
    detail: substantive
      ? '检测到实质文件变更'
      : '未检测到实质产出，疑似机械回复 / 空转完成，不得标记为完成',
  };
}

async function runPreRegisteredTests(
  pkg: ExecutorTaskPackage,
  jobEvidenceDir: string,
  preRegisteredTests?: Array<{ command: string[]; description?: string; cwd?: string }>,
): Promise<VerificationCheckResult> {
  const tests = preRegisteredTests || [];
  if (tests.length === 0) {
    return {
      id: 'pre_registered_tests',
      title: '预注册验证是否通过',
      verdict: 'unverifiable',
      detail: '未提供预注册验证（not_registered），不执行',
    };
  }
  const logsDir = path.join(jobEvidenceDir, 'pre-registered-test-logs');
  await fs.mkdir(logsDir, { recursive: true });
  const results: Array<{ description: string; exitCode: number | null; passed: boolean }> = [];
  for (const [i, t] of tests.entries()) {
    const t0 = Date.now();
    const r = runCommandShellFalse({
      command: t.command,
      cwd: t.cwd || pkg.workingDirectory,
      timeoutMs: 180_000,
      env: { ...process.env, npm_config_yes: 'true' },
    });
    const logRel = `pre-registered-test-logs/${i + 1}-${(t.description || 'test')
      .replace(/[^\w.-]+/g, '_')
      .slice(0, 60)}.log`;
    await fs.writeFile(
      path.join(jobEvidenceDir, logRel),
    ...
    results.push({
      description: t.description || r.commandLine,
      exitCode: r.status,
      passed: r.status === 0,
    });
  }
  ...
  return {
    id: 'pre_registered_tests',
    title: '预注册验证是否通过',
    verdict: allPassed ? 'satisfied' : 'unsatisfied',
    detail: allPassed
      ? `预注册验证通过：${results.map((r) => r.description).join('；')}`
      : `预注册验证失败：${results
          .filter((r) => !r.passed)
          .map((r) => `${r.description} → exit ${r.exitCode}`)
          .join('；')}`,
  };
}
