...
`user_feedback_loops_accepted` / `not_mvp_ready` / `not_production_ready` / `not_pushed`

分支：`build/subject-learning-availability-01`。收口前 HEAD：`eef1eb4`。工作区：`D:\Projects\Digital Me`。

### 本轮已完成（Owner 已接受）

1. **对话滚动**：工作对话纵向可滚、发送钮保持在可视区。
2. **主体资料**：上传资料进入可检索索引，跨对话与做事可用；删除后失效。
3. **公开网页 / GitHub**：只读网页走安全 HTTP；反斜杠 URL 可解析；Releases 与远程审计分流；失败保留 Task/Job，不写伪成果。
4. **DNS/重定向 SSRF**：连接前解析全部 A/AAAA 并钉死 IP；Host/SNI 用原域名；每次重定向重校验。生产 `BingHtmlSearchConnector.read()` 不再用未保护 `fetch`。`localtest.me` 解析为 `127.0.0.1` 时本机监听服务命中数为 0；公开地址 302 到 `localtest.me` 同样为 0。
5. **PPT 语义版式**：航天工程交叉生物学样例含流程节点与箭头、时间轴、对照矩阵、KPI 卡、结论行动区；第 4 页交叉说明深色可读；第 6 页无行首孤立顿号。不是杂志级演示，但是语义图形而非纯文字换位。

### 验证

1. 收口前：`git diff --check`、`npm run build`、本轮专项测试（见提交说明）。
2. 最近一次完整 `npm test`（SSRF/PPT 专项通过后）：**1005** / **970** pass / **29** fail / **6** skip。失败集合与修改前相同。
3. 收口时不重跑全量，不为既有 29 项失败改无关代码或降低断言。

### 边界

1. **未推送**远程。
2. **`digitalme-v2/` 未纳入、未改动。**
3. 安装包按仓库现有 `npm run build:packaged`（win x64 zip）生成，不覆盖旧 staging 目录。
4. GENERAL-TASK-CLOSURE-01 仍为规划、未实施。本提交满足其独立提交启动条件后，才可做 P0。

### 安装新版后的最小现场验收

1. 工作对话长内容可滚、发送钮可见。
2. 上传资料后，新对话与做事能用到资料要点。
3. 访问公开网页 / GitHub 公开仓：成功有摘录，失败有分类、无伪成果。
4. 导出航天样例 PPT，WPS 检查第 4 页交叉可读、第 6 页无行首「、」。

---

## 2026-08-30 · 使用反馈修复（代码与自动化完成 · Owner 现场验收未完成）

### 本次目标

在当前分支 `build/subject-learning-availability-01`（HEAD `eef1eb4`）落实用户反馈修复：GitHub 公开仓审计须真正读取主要源码；任务对话身份捕获须按本轮 `turnId` 幂等；补齐自动化回归证据。只改工作区，**不提交、不推送、不打包**；不触碰未跟踪目录 `digitalme-v2/`。

### 已完成

1. **代码与自动化**：GitHub 有预算递归抓取（Trees API / 深度受限 Contents）、覆盖说明、下载 URL 主机校验；身份 `captureKey` 改为 `work-converse-identity:${userTurnId}`；姓名纠正进入 supersede；会话路径与当前对话 transcript 对齐。
2. **规划文档**：[`docs/plans/GENERAL-TASK-CLOSURE-01.md`](docs/plans/GENERAL-TASK-CLOSURE-01.md)（`planned / not_implemented`）；`docs/architecture/README.md` 已加链接。
3. **明确未完成**：Owner 现场验收（WPS / 真实 GitHub 网络 / Electron 界面）**尚未开始**。本条**不是** `owner_accepted` / `mvp_ready` / `production_ready`。

### 自动化证据

1. `npm run build` 通过。
2. 专项复核 **28/28** 通过。
3. 完整 `npm test`（会话路径对齐后重跑）：**962** 项、**928** 通过、**29** 失败、**5** skipped、**0** cancelled、**0** todo、**97** suites；耗时约 **437893 ms**。
4. 当前 29 项失败全部位于干净 HEAD 的既有失败集合内，没有新增失败。
5. 干净 HEAD 失败而当前通过的 3 项：规划水合接线、lookup 只读材料选择、回流失败不影响已成功 Artifact。
6. 会话路径对齐后另外修复的 2 项：拒绝长期了解时不提取回答正文、自然语言「不要记住」能阻断长期提取。

### 未完成与边界

1. **代码和自动化完成 ≠ Owner 现场验收完成。** 不得把本轮写成已验收或可发布。
2. 当前改动未提交、未推送、未打包。
3. `digitalme-v2/` 仍为未跟踪目录，未删除、移动、覆盖或纳入改动。
4. GitHub 审计专项兜底**没有**覆盖所有未知任务类型；未知任务仍可能落入 `general → document`。
5. 完整测试剩余 29 项失败与干净 HEAD 同类（真实模型 e2e/评测、`predicted=null`、缺失 Electron/启动脚本、Coding 能力引导等），本轮不为全绿修改无关测试。

### 明日第一步

Owner 现场验收，按序：

1. WPS / Word / PowerPoint 打开 DOCX / PPTX，检查中文、换行和溢出。
2. 真实 GitHub 网络审计及源码覆盖说明。
3. Electron：会话、姓名纠正、简历确认、失败提示词、重试、协作布局。

现场通过并完成最终全量回归核对后，本轮使用反馈修复再形成**独立提交**。提交完成前不得启动下一阶段代码实施。

### 下一阶段

[`docs/plans/GENERAL-TASK-CLOSURE-01.md`](docs/plans/GENERAL-TASK-CLOSURE-01.md)

- 状态：`planned / not_implemented`
- 启动条件：本轮使用反馈修复完成明日现场验收并形成独立提交后**立即启动**
- 第一阶段：**P0 通用外部交接闭环**
- P0 验收后才做 **P1 受控能力发现**
- P1 稳定后才做 **P2 可信工具目录与安装引导**
- 不得把现有 GitHub 专项兜底描述成已经覆盖所有未知任务

---

## 2026-08-18 · 产品主语义修复（DIGITALME-PRODUCT-SEMANTICS-RECOVERY-01）

### 状态

`product_semantics_recovery_candidate` / `integrated_owner_runtime_accepted` / `owner_runtime_accepted` / `not_market_p95_met` / `not_mvp_ready` / `not_pushed` / `not_merged`

工作树：`D:\Projects\dm-control-layer-integration-01`。分支：`build/control-layer-integration-01`。基线：`f361dfb`。

### 根因（同一条链，不是六个独立 UI 缺口）

1. 对话资料查询走了 `submitTask`，MCP 调用被当成创建任务。
2. `updateForSubmit` / 上下文合并会用后一个 goal 和目录覆盖当前 Task 身份。
3. 有成果时 `confirm_start` 一律当修订；模型把「采用。」判成开始修改。
4. 用户面把技术验收推给 Owner（「请决定是否采用」「请判断是否正确」）。
5. 做事成功未写入现有 growth / `task_requirement` 回流；外部资料查询若建 Task 还会污染本人事实。

### 修复

1. 对话 lookup 直接调用只读资料能力，不创建 Task / Job / 成果卡。
2. 已有 Task 冻结 taskId、原始 goal、已绑定工作目录；无关目标必须新 Task。
3. CTO 通过后用户面为「已经完成修改并检查通过。」不要求 Owner 做技术判断。
4. 当前已通过成果上，「采用。」结束本轮，codingJobCount 增量为 0；「继续修改……」才允许新 Job。
5. 成功做事经历走现有 `captureInput` / growth event；Project Alpha 等外部资料不进入本人事实。

### 验证

1. 定向回归 A–G：`product-semantics-recovery-01` 通过。
2. Growth context consistency 01 / 01b / 01c / 01d 通过。
3. `gate:real-agent-doing-01`（AtomCode）、`gate:second-real-agent-01`（OpenCode HTTP）、`gate:real-mcp-readonly-01` 通过。
4. External Capability Contract 单测与骨架、Multi-Agent Route、MCP readonly fixture gate、smoke / model-gate / Electron preflight 通过。

### 边界

1. **2026-08-19 Owner 真机通过**本集成候选的做事主路径与刚完成项目回流。`integrated_owner_runtime_accepted = true`。范围不含 `mvp_ready`、市场 95 分位、`v2/foundation` 协作收口。
2. 未新增 Task Store、长期事实 Store、关键词总路由、第三 Agent、MCP 或 P95。
3. 未 push / 未 merge。
4. Owner 同时指出细节仍需持续打磨；打磨项入 backlog，不自行开下一块。

### Owner 再测后的跟进（同日）

1. 做事经历写入「近期完成的工作」，进入现有 `userVisibleFacts` 与对话系统提示。问「你了解我什么」仍走事实清单；「了解增加了吗 / 通过什么方式」不再走 01C 固定句，也不另造答题模板。
2. 时间线会把错位的「本次任务结束」挪到末尾；「无，可直接交付使用」不再单独成条。
3. 「新建任务」清空上一任务时间线与成果，空白起草不再显示「正在理解你的目标」。
4. 规划阶段把项目里的 `.js` / `.json` 当普通文本读取，不得再说「工具不支持该格式」。本轮相对快照无新改动时，范围内相对 HEAD 的未提交改动仍可进入采用。对话「我是在修改…项目」记为最近在做的事，不当成身份。
5. **2026-08-19 Owner 真机通过**：原任务完成并采用；2digime 能提到刚完成的开发项目。细节打磨入 backlog。

---

## 2026-08-19 · 集成真机验收（DIGITALME-CONTROL-LAYER-INTEGRATION-01）

### 状态

`integrated_owner_runtime_accepted` / `owner_runtime_accepted` / `details_polish_backlogged` / `not_market_p95_met` / `not_mvp_ready` / `not_pushed` / `not_merged`

工作树：`D:\Projects\dm-control-layer-integration-01`。分支：`build/control-layer-integration-01`。验收实现 HEAD：`948ead1`。

### Owner 真机结论

1. 原任务完成（显示名空格修复并采用）。
2. 2digime 知道刚完成的开发项目。
3. Owner 明确「可以通过」。
4. 细节仍需持续打磨，不视为本轮未通过。

### 边界

1. 仅本集成工作树「对话—做事—采用—回流」主路径。
2. **不是** `mvp_ready` / 市场 95 分位 / 协作扩展已开放。
3. `v2/foundation` `a851b0c` 协作最小收口仍 **不是** Owner accepted。
4. 未 push / 未 merge。下一项 = 细节打磨待 Owner 点名。

---

## 2026-08-18 · 第二真实 Coding Agent（DIGITALME-SECOND-REAL-AGENT-01）

### 状态

`second_real_agent_validated` / `not_owner_runtime_accepted` / `not_market_p95_met` / `not_pushed` / `not_merged`

工作树：`D:\Projects\dm-control-layer-integration-01`。分支：`build/control-layer-integration-01`。基线：`1c350b3`。

### 验证

1. OpenCode CLI 1.18.18；从隔离库存仓库启动 `opencode serve`（`127.0.0.1` + 随机端口 + `mdns=false` + 运行时 `OPENCODE_SERVER_PASSWORD`）。`GET /global/health` 返回 healthy + version `1.18.18`；无密码为 401。`GET /doc` 为 OpenAPI 3.1（162 paths）。
2. 正式 Job 经 HTTP：`POST /session`、`POST /session/{id}/message`（parts + 内部模型），并读 `/event`、`/session/{id}/message`、`/session/{id}/diff`。**未**对正式 Job 调用 `opencode run`。
3. 路径：用户目标 → 2digime 规划确认 → 显式 `cap_external_executor_secondary` → 恰好 1 个 Coding Job → 真实改 `src/inventory.js` / `test/inventory.test.js` → 独立复测 exit 0 → 验收 `accepted`。AtomCode 仍可用但本 Job 未被调用；无自动 fallback / 无第二 Job。
4. Agent identity = OpenCode；connector = HTTP/API；内部模型 `opencode-go/kimi-k2.7-code` 不进入 identity。与 AtomCode CLI 共用同一上层 request/result/job 合同。
5. 用户面成功文案：「已经完成修改并检查通过。」回归：REAL-AGENT-DOING-01、REAL-MCP-READONLY-01、合同/路由/MCP readonly、成长一致性、smoke / model-gate / Electron preflight 通过。

### 边界

1. 默认产品 runtime 仍不注册备用能力。仅本轮显式 options 启用。
2. **不是**自动选 Agent、自动 fallback、marketplace、Owner 真机验收或 `market_p95_met`。
3. 未 push / 未 merge。不进入 P95。

---

## 2026-08-18 · 真实只读资料查询（DIGITALME-REAL-MCP-READONLY-01）

### 状态

`real_mcp_readonly_validated` / `ready_for_second_real_agent` / `not_owner_runtime_accepted` / `not_market_p95_met` / `not_pushed` / `not_merged`

工作树：`D:\Projects\dm-control-layer-integration-01`。分支：`build/control-layer-integration-01`。基线：`3fd5713`。

### 验证

1. 官方 `@modelcontextprotocol/server-filesystem` 经 `npx -y` 启动（未全局安装）；`initialize` 返回 `secure-filesystem-server` 0.2.0 / 协议 `2025-03-26`。
2. 真实 `tools/list` 含读写两类工具。Digital Me 投影仅：`read_file` / `read_text_file` / `list_directory` / `search_files` / `get_file_info` / `list_allowed_directories`。写工具不可见。
3. 2digime 对自然语言目标选择 `cap_mcp_readonly`，经真实 `tools/call` 读取隔离 `notes/`，用户面：「Project Alpha 处于 active 状态，优先级为 high。」
4. `write_file` 在控制层拒绝，代码 `rejected_by_digitalme_policy`，未发往外部；`project-alpha.md` / `project-beta.md` hash 前后一致。
5. 原 MCP-READONLY fixture gate、合同/路由/成长一致性、Real Agent Doing gate、smoke / model-gate / Electron preflight 通过。

### 边界

1. 默认产品 runtime 仍不注册该能力。仅本轮显式 options 启用。
2. **不是**通用资料源市场、OAuth、写能力、网络资料源或多个第三方服务。
3. 未启动第二 Agent / P95。未 push / 未 merge。

---

## 2026-08-18 · 真实 Agent 做事主路径（DIGITALME-REAL-AGENT-DOING-01）

### 状态

`real_agent_doing_validated` / `ready_for_real_mcp_validation` / `not_owner_runtime_accepted` / `not_market_p95_met` / `not_pushed` / `not_merged`

工作树：`D:\Projects\dm-control-layer-integration-01`。分支：`build/control-layer-integration-01`。基线：`067f721`。

### 验证

1. 本机 AtomCode 5.0.6 CLI 已登录；`--version` / `status` / 无文件副作用 ping（`PONG`）通过。
2. 2digime converse → 规划确认 → 恰好 1 个 Coding Job → AtomCode 修改隔离 git 仓库 `src/format.js` 与 `test/format.test.js` → 独立复跑测试 exit 0 → 验收 `accepted`。Job 状态 `succeeded`。未自动派第二次 Job。
3. 用户面成功文案：「已经完成修改并检查通过。」
4. 定向单测 + 既有六块回归 + smoke / model-gate / Electron preflight 通过。全量 `npm test` 历史失败项不视为本轮回归。

### 边界

1. 默认产品路径仍探测 Codex；本轮通过 `cliKind: atomcode` 显式接通，**不是**全体用户默认执行器切换。
2. 规划步使用既有做事合同结构化 converse；不得把额度耗尽的 Codex 标为成功。
3. **未**接第二 Agent、**未**跑 P95。同日稍后真实只读资料查询已另块验证。**不等于** Owner 真机验收 / `mvp_ready`。
4. 未 push / 未 merge。原工作树与 `v2/foundation` 未改。

---

## 2026-08-18 · 控制层产品集成候选（DIGITALME-CONTROL-LAYER-INTEGRATION-01）

### 状态

`control_layer_integrated_candidate` / `growth_owner_acceptance_preserved` / `control_layer_engineering_regression_passed` / `ready_for_real_agent_doing_validation` / `integrated_owner_runtime_accepted` / `not_market_p95_met` / `not_pushed` / `not_merged`

工作树：`D:\Projects\dm-control-layer-integration-01`。分支：`build/control-layer-integration-01`。基线：`bfb70bb`（祖先含 `a3b773d`）。

### Git 拓扑

1. 情况 A：`a3b773d` 是 `bfb70bb` 的祖先；五块为真实线性链，未 cherry-pick。
2. 纳入提交：`f4fe829` EXTERNAL-CAPABILITY-CONTRACT-01 → `86462d8` CODEX-DOING-CLOSED-LOOP-01 → `fb375a0` MCP-READONLY-ADAPTER-01 → `95bfe30` MULTI-AGENT-ROUTE-01 → `bfb70bb` MARKET-P95-CANDIDATE-GATE-01。
3. 不含 `v2/foundation`。无冲突。未改写历史。

### 验证

1. 成长事实 01/01b/01c/01d Electron 验收通过；页面与对话共用当前有效本人事实；普通知识仍进模型；本人事实不足不得自由推断；清空对话不清主体事实。
2. 五块定向单测 + hook gate 通过。P95 REAL 无预算 = `scoring_blocked_missing_budget`，未标 `market_p95_met`。
3. smoke / model-gate / Electron preflight 通过。全量 `npm test` 中历史 D11/真实模型评测失败项在 `a3b773d` 已存在或依赖真实模型，不视为本轮集成回归。

### 边界

1. **集成候选工程回归通过 ≠ 已达市场 95 分位 ≠ `mvp_ready`**。2026-08-19 本工作树「对话—做事—采用—回流」主路径已由 Owner 真机通过，见上条 08-19 验收节。
2. 当时未启动 `REAL-AGENT-DOING-01`（同日稍后已在本工作树验证）。未 push / 未 merge。原工作树未改。

---

## 2026-08-18 · 市场 95 分位候选门（MARKET-P95-CANDIDATE-GATE-01）

### 状态

`p95_candidate_gate_engineered` / `engineering_accepted` / `local_feature_commit_created` / `protocol_ready_or_blocked` / `not_p95_met` / `not_owner_accepted` / `not_pushed` / `not_merged`

工作树：`D:\Projects\dm-market-p95-candidate-gate-01`。分支：`build/market-p95-candidate-gate-01`。基线：`95bfe30`。

### 工程交付

1. 封闭终态：`protocol_ready` / `scoring_blocked_missing_budget` / `scoring_blocked_missing_blind_review` / `scoring_blocked_agents_unavailable` / `candidate_scored_not_claimed`。禁止 `p95_met` / `mvp_ready` / `closed_alpha_ready`。
2. 等预算、盲评去臂、拆封、可复现哈希均有单测；复用 P0 fixture 与合同指标公式。
3. 默认 gate = `protocol_ready`；REAL 无预算授权 = `scoring_blocked_missing_budget`，exit 0 且不宣称达标。
4. CTO 记录：hook 三项前置为协议自检而非真实授权；证据指标为 `not_run` 占位；`zeroExternalCalls` 极性易误读；无实际消耗字段故超预算不得 completed 未落成断言。

### 边界

1. **本块通过 ≠ 已达市场 95 分位 ≠ Owner accepted**。
2. 无真人盲评、无等预算真实对照、无昂贵模型评测。
3. 仍 **未 push / 未 merge**。控制层六块工程序列已收口。下一步 = 整系统阶段复盘决策门。

---

## 2026-08-18 · 第二 Agent 显式路由（MULTI-AGENT-ROUTE-01）

### 状态

`multi_agent_route_engineered` / `engineering_accepted` / `local_feature_commit_created` / `secondary_hook_skeleton_only` / `not_owner_accepted` / `not_pushed` / `not_merged`

工作树：`D:\Projects\dm-multi-agent-route-01`。分支：`build/multi-agent-route-01`。基线：`fb375a0`。

### 工程交付

1. `routeCodingAgent`：主可用选主；主不可用备可用为 `fallback_explicit`；都不可用诚实失败，不回退写作。
2. `selectForNeed(modify_code)` 走路由，不再「Codex 否则任意 code-change」。
3. 备用代码执行能力默认不注册；`isCodingJob` 识别主备。
4. CTO 记录：确认预览仍走偏好评分；Job 只记 capabilityId；备用不是第二套独立验收闭环。

### 边界

1. **engineering accepted ≠ Owner accepted**；不得宣称 Claude/Cursor 已可用。
2. 仍 **未 push / 未 merge**。
3. Owner 已批准启动任务块 6：`MARKET-P95-CANDIDATE-GATE-01`。通过该块 **不得** 宣称已达市场 95 分位。

---

## 2026-08-18 · 最小只读工具能力接线（MCP-READONLY-ADAPTER-01）

### 状态

`mcp_readonly_wired` / `engineering_accepted` / `local_feature_commit_created` / `fixture_protocol_not_full_mcp` / `not_owner_accepted` / `not_pushed` / `not_merged`

工作树：`D:\Projects\dm-mcp-readonly-adapter-01`。分支：`build/mcp-readonly-adapter-01`。基线：`86462d8`。

### 工程交付

1. `mcp-stdio` 进入 `ADAPTER_TYPES`；`assertMcpWireableGuard` / `assertMcpRegistrationShape` 取代 declared-only 门。
2. 只读适配器：`kind=tool`，权限仅 `filesystem_read`，用户面「资料查询能力」。
3. 默认 runtime 不注册；显式 `mcpReadonlyCapability` 才注册。
4. fixture：`list_notes` / `lookup_note` 成功；`write_note` 拒绝且不落盘。
5. CTO 记录：逐行 JSON-RPC fixture ≠ 完整 MCP 帧协议；真实第三方未接，不假绿。

### 边界

1. **engineering accepted ≠ Owner accepted**。
2. 不自动启动市场 95 分位基准。仍 **未 push / 未 merge**。
3. Owner 已批准启动任务块 5：`MULTI-AGENT-ROUTE-01`。

---

## 2026-08-18 · Codex 做事闭环工程收口（CODEX-DOING-CLOSED-LOOP-01）

### 状态

`codex_doing_closed_loop_engineered` / `engineering_accepted` / `local_feature_commit_created` / `real_codex_not_available` / `not_owner_accepted` / `not_pushed` / `not_merged`

工作树：`D:\Projects\dm-codex-doing-closed-loop-01`。分支：`build/codex-doing-closed-loop-01`。基线：`f4fe829`。

### 工程交付

1. 独立 verifier 增加 `substantive_output` 与 `pre_registered_tests`；后者失败为 hardFail，未提供为 unverifiable。
2. verifier `unsatisfied` 时 Codex 适配器不得完成；同一执行阶段第二 Coding Job 拒绝。
3. hook 对照：4×2=8 条 arm；编排臂拦截机械回复与假研究完成，隔离 IMPRINT 改动并跑声明测试，中断后同 taskId 恢复。
4. CTO 最小补丁：hardFail 与设计对齐；real 模式不再套用 hook 的 8 条 arm / 公式断言。
5. 场景 4「显式采用」由既有 closed-loop 测试覆盖（`captureSubjectInput(artifact_acceptance)`）。

### 边界

1. hook 对照臂是 naive 镜像，不是真实 Codex 直连。
2. 本机 Codex 未安装，`DIGITALME_CDCL_REAL=1` 写 limitation 后回退 hook，不假绿。
3. **engineering accepted ≠ Owner accepted**；不得宣称 `mvp_ready` 或市场 95 分位。
4. 不自动启动 MCP、第二 Agent 或市场基准。仍 **未 push / 未 merge**。

---

## 2026-08-18 · 外部能力合同冻结与 Codex 做事闭环启动

### 状态

`external_capability_contract_engineered` / `external_capability_contract_frozen` / `engineering_accepted` / `local_feature_commit_created` / `codex_doing_closed_loop_started` / `not_pushed` / `not_merged`

任务块 2 工作树：`D:\Projects\dm-external-capability-contract-01`。分支：`build/external-capability-contract-01`。基线：`a3b773d`。任务块 3 工作树：`D:\Projects\dm-codex-doing-closed-loop-01`。分支：`build/codex-doing-closed-loop-01`。

### 任务块 2 收口（EXTERNAL-CAPABILITY-CONTRACT-01）

1. 四类连接器（`model` / `agent` / `mcp-tool` / `a2a-agent`）、两条对照臂（`direct` / `orchestrated`）、4 条 P0 fixture、预注册指标与增益公式均已机器可读化。
2. `external-connector-contract.ts` + 12 项单测；`bench:external-capability-skeleton` 8 条 `not_run` 占位 + 公式校验通过。
3. CTO 独立复验：`build` / `smoke` / 合同单测 / 既有 capability 测试均绿；主工作区 `D:\Projects\Digital Me` 未动。
4. **零真实外部调用**；`mcp-stdio` 未进入生产 `ADAPTER_TYPES`。
5. 边界：本块 **不等于** 做事主路径已走通；**不得** 宣称市场 95 分位。

### 任务块 3 启动（CODEX-DOING-CLOSED-LOOP-01）

Owner 2026-08-18 批准：以现有 `external-executor-codex` 为首个成熟 Agent，重建可验证、可恢复的做事主路径；独立 verifier 必须跑预注册测试；同一 P0 任务做 Codex 直连 vs 2digime→Codex 对照；**不得** 整包合入 Kernel-34。

### 边界

1. 工程 accepted 仅覆盖任务块 2 合同冻结范围，不表示 Codex 闭环已完成或 `mvp_ready`。
2. 仍 **未 push、未 merge** 到 `v2/foundation`。
3. 任务块 3 不得并行启动 MCP、第二 Agent 或市场 95 分位基准。

---

## 2026-08-17 TODAY-CLOSE · 系统复盘与成长事实一致性 Owner 收口

### 状态

`system_stage_reviewed` / `doing_strategy_reoriented_to_external_ai_orchestration` / `growth_context_consistency_engineered` / `real_model_validated` / `owner_runtime_accepted` / `local_feature_commit_created` / `not_pushed` / `not_merged`

候选工作树：`D:\Projects\dm-growth-context-consistency-fix-01`。分支：`build/growth-context-consistency-fix-01`。验收实现提交：`2318d50`。任务链：`GROWTH-CONTEXT-CONSISTENCY-FIX-01A～01E`。

### 阶段复盘结论

1. 「做事」尚不能仅凭既有工程测试宣称达到市场 95 分位；后续需要等预算、可复现、盲评的产品级基准。
2. 2digime 不重复开发成熟 AI 产品已有的核心功能；主体层重点建设意图理解、能力选择、上下文裁剪、授权、委派、验收、失败恢复、采用回流与主体反馈。
3. 后续候选顺序保持为：外部能力连接器合同与同 Agent 对照 → Codex 首个成熟 Agent 做事闭环 → 最小只读 MCP → 第二成熟 Agent 与路由降级 → 市场 95 分位候选基准。每次只推进一个经批准任务块。

### 真实阻断与根因

“数字之我”页面已经显示本人事实，但普通对话没有装配同一事实来源，模型因此可能声称一无所知；早期修复又先后出现页面与模型事实筛选不一致、普通知识问题被推断检测器误拦、本人推断查询分类器范围过宽等回归。

### 01A～01E 最终实现

1. `buildUserVisibleFacts` 成为页面「已经了解」、成长视图、对话上下文和受控回复的唯一事实投影源。
2. 只保留已确认、当前有效、非资产、非临时、非外部主张、非内部过程且含具体值的本人事实；数量与字符预算在同一来源完成。
3. 主体上下文读取失败不得降级为空事实继续调用模型；检测异常 fail closed。
4. “关于我，你有什么印象”等主体事实查询，以及明确询问本人属性的高精度句式，使用事实受控回复，不调用模型自由推断。
5. “求职是什么”“职业中期有什么挑战”及普通知识/工作请求继续调用模型。
6. 分类器最终移除“第一人称词与主题词任意共现”、任意字符窗口和单字“算”等误判来源；真实门按每次请求最后一条用户消息核对模型调用。
7. 清空对话仅清会话，不清除主体事实。

### 验证与 Owner 验收

- build、smoke、01A～01E 定向单测与 Electron 验收通过。
- growth-system、guided conversation、chat clear 回归通过。
- 隔离用户数据下完成真实 DeepSeek 对照；知识问题进入模型，本人事实/推断查询不进入自由生成；证据目录未包含 SubjectPackage、events、materials、成长或运行时个人数据。
- 2026-08-17 20:57 后打开当前 01E 修复版；Owner 按真实 Electron 主路径检查页面事实、主体事实回复、本人推断、一般知识和清空后事实保留，随后明确反馈：**“通过。”**
- 最终状态：**`owner_runtime_accepted` / `accepted_on_local_feature_branch`**。

### 边界与今日停止

1. Owner accepted 仅覆盖本次成长事实一致性与回复门真实路径，不表示候选改动已经进入 `v2/foundation`，不表示 `mvp_ready` 或市场 95 分位已达成。
2. 验收实现已本地提交为 `2318d50`；仍未 push、未 merge。
3. 今日到此停止；不自动启动外部连接器、Codex、MCP、第二 Agent、市场基准或协作扩展。
