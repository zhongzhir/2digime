...
<parent>/
  ...
  .digitalme-pkgstore/
    <packageName>/
      lock.json                  # 持锁期不可替换的所有权文件
      lock-heartbeats/           # heartbeat 旁路（不触及 lock.json）
      journals/
        journal-0000000001.json  # 当前未完成事务内的 journal 序列
        journal-0000000002.json
      changesets/
...
1. 获取单写者锁（`open("wx")` + 不可预测 `operationToken`；actor 仅逻辑身份；同 actor 第二进程亦阻断；stale 仅 PID 死亡后原子 rename 接管）；
...
- **journal**：`journals/journal-NNNNNNNNNN.json`；在**当前未完成事务的 journal 序列内**单调递增 generation；每次 phase 写 publishing 再 rename 发布；崩溃时上一完整 generation 仍可读；recover 会提升完整但未 rename 的 publishing。事务成功结束后清理该序列。
...
| 能力 | 证明方式 |
|---|---|
| 崩溃不丢旧版 | journal 记录 phase；`recover()` 按摘要/revision 矩阵恢复 |
| 半写不落 live | 变更只发生在 staging；成功前不改名覆盖 live 内文件 |
| 并发单写者 | lock.json + pid；第二写者失败 |
