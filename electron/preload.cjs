...
contextBridge.exposeInMainWorld("digitalMe", {
  ...
  talkUiDeadlineMs: Number(process.env.DIGITALME_V2_TALK_UI_DEADLINE_MS) || 620000,
  invoke(name, input) {
  ...
  getInstitutionStatus: () => ipcRenderer.invoke("shell:getInstitutionStatus"),
  connectInstitution: (input) => ipcRenderer.invoke("shell:connectInstitution", input || {}),
