...
状态：正式需求源（界面与功能）
日期：2026-07-19
关联：`digitalme_context.md`（战略与架构）、`digitalme_phase1_task_P1-PANORAMA_product_panorama_alpha.md`（当前总任务，v0.4 三位一体）、`digitalme_phase1_task_PAN-00R_three_part_alpha_reset.md`（三位一体重构与极简原则冻结）、`digitalme_panorama_execution_index_v0.1.md`（当前执行索引）、`digitalme_phase1_task_PAN-01R_sovereign_collaboration_loop.md`（历史任务包；协作骨架保留为内部验证器）、`digitalme_digital_sovereignty_narrative_v0.1.md`（数字主权叙事）、`digitalme_phase1_subject_upgrade_plan_v0.1.md`（Trusted Beta 硬化依据，不再是当前执行队列）、`digitalme_architecture_audit_20260716.md`（审计风险基线）、`digitalme_person_definition_v0.1.md`（人的定义与采集口径）、`digitalme_inbox_access_plan_v0.1.md`（投递箱与可读范围）、`digitalme_architecture_edge_sovereign_v0.1.md`（部署拓扑）、`digitalme_narrative_ai_era_autonomy.md`（对外叙事）、`digitalme_intake_questionnaire.md`（自我评测）

> **当前版本**：**v0.6.1**（2026-07-19，PAN-00R Codex 第一轮最小文档修复；主线仍为 v0.6 三位一体与极简原则，见 §2.0 / §2.2 / §7.6 / §10）。v0.5.x / v0.6 历史修订记录全部保留。
---
...
默认侧栏只保留主要导航与确有必要的阻断性状态，不强制常驻 Package 摘要、模型状态、能力摘要或品牌句。Package 与模型细节进入设置／高级；能力信息进入能力页；品牌句可由 PAN-01S 按极简原则删除。具体保留项由 PAN-01S 独立任务包冻结。

普通用户默认界面**不得**要求常驻：Package 版本或健康细节、模型连接技术状态、已武装能力摘要的重复展示、品牌口号、工程状态。

**v0.6 / v0.6.1 变更（PAN-00R；界面收口由 PAN-01S 落实）**：

- 「产品全貌首页」不再作为「我」的默认表面；PAN-01 密集信息展板裁定为产品感知失败，默认「我」页面由 PAN-01S 收口为极简表面；
- 四个承诺、成长路线与证据链说明迁为**帮助内容**，不作为默认首页主体；
- PAN-01R「体验一次 Digital Me 如何代表我」五步体验撤出普通用户路径，保留为高级/开发者**协作回路验证器**；
...
> **v0.6 状态注记（2026-07-19，PAN-00R）**：本节保留为 v0.5 历史规格。其中安全边界（§7.5.10）、五态标签（§7.5.8）、结果处置规则（§7.5.7）**继续有效**；「产品全貌首页」「四承诺/成长路线作为首屏主体」「PAN-01R 五步体验作为普通用户产品证明」「旧 PAN-02 控制权面板」等要求已被 §2.0 / §3.1 / §7.6 取代。当前排期见执行索引 v0.2；下一任务为起草 PAN-01S 独立任务包。
...
**当前唯一执行顺序**以 `digitalme_panorama_execution_index_v0.1.md`（文内 v0.2）为准；当前唯一下一任务为**起草 PAN-01S 独立任务包**（本轮 PAN-00R 为文档任务，不改代码；PAN-02～PAN-06 未开始）。
