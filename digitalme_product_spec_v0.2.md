...
> **当前版本**：**v0.6.3**（规格文本冻结于 2026-07-19；主线仍为 v0.6 三位一体与极简原则）。**实现与验收状态（2026-07-20）**：PAN-01S / PAN-01S.1 / PAN-01S.2 已 `accepted`（Owner real Electron runtime；baseline `cbde807`）。本规格文本本身不因验收升版。当前排期见执行索引 v0.2.7；**下一门槛**：PAN-02 = `planned` / `blocked`；renderer foundation R0 = `planned` / `not_started`。v0.5.x / v0.6 / v0.6.1 / v0.6.2 历史修订记录全部保留。
---
...
| 高级 / 开发者工具 | Package 版本和健康细节、证据来源与分类、DecisionAudit、完整授权记录、推理环境、调试信息（**不含** PAN-01R 入口；PAN-01R 仅内部 test harness，见 §3.1.1） |
...
- **v0.6.3（2026-07-19，PAN-01S Codex 第一轮）**：冻结主操作唯一优先级 P0→P4（互斥、顺序求值）；废止模糊「未完成构建」条件；冻结 PAN-01R 无生产入口（设置／高级／帮助亦不提供）；test-only 仅隔离 harness。详见 §3.1.1 与任务包 v0.1.1。**不**表示已实现。
...
- **v0.6（2026-07-19，PAN-00R）**：三位一体重构与极简产品原则冻结——§2.0 极简产品原则与前台/后台/帮助/高级分层；§2.0.1 主体信息分层使用规则；§3.1 当前 IA 改为极简工作表面；四承诺、成长路线、证据链说明迁为帮助内容；PAN-01R 五步体验历史化为内部协作回路验证器（普通用户入口由 PAN-01S 撤下；**v0.6.3 进一步明确：不进入生产设置／高级，仅内部 test harness**）；§7.6 第一阶段三位一体 Alpha DoD；下一任务改为起草 PAN-01S 独立任务包。
...
└── 设置                 模型网关 / Package 路径 / 高级与开发者工具（不含 PAN-01R 入口）
```
...
#### 3.1.1 默认「我」极简主体入口（v0.6.3 · PAN-01S 冻结；实现见任务包 v0.1.1）

点击侧栏「我」后，默认进入极简主体入口（非全貌展板、非构建页、非 PAN-01R 体验）。

**首屏允许**：本人 Digital Me 名称；一句不超过两行的诚实当前说明；一个主操作；最多一个次操作；仅在需要本人处理时的一条短提醒。

**主操作优先级（主进程可信状态派生；互斥；按 P0→P4 顺序求值；命中即停；renderer 不得覆盖或重算）**：

| 优先级 | 条件 | 主操作 | 路由 |
|---|---|---|---|
| P0 | 读取异常：`read_error`／`content_degraded`／结构损坏／不能安全给出正常主体结论 | `查看问题` | 既有设置／问题入口；`navTarget` 不存在则不导航 |
| P1 | Package `missing`／`uninitialized`（且非读取损坏） | `继续构建` | 既有构建入口 |
| P2 | `awaiting_review`／`pending_confirmation` | `继续确认` | 既有待审阅深链 |
| P3 | 可操作 inbox：`suggested`／`failed-retryable` 等（**不含** `processing`） | `继续完善` | 既有构建入口 |
| P4 | 主体正常且无以上待办 | `查看我的信息` | 既有数字之我详情 |

主进程输出最终：`primaryAction`、`primaryActionLabel`、`primaryNavTarget`、optional `secondaryAction`、optional `reminder`。优先复用 SubjectOverview／panorama 现有只读字段；不足时允许最小扩展现有只读契约，**不得新增平行 IPC**。

> **废止（historical / superseded）**：v0.6.2「有待处理材料或未完成构建」等可重叠条件表；模糊「未完成构建」条件。

**冲突案例（冻结）**：`awaiting_review`+`suggested`→`继续确认`；`read_error`+`awaiting_review`→`查看问题`；`missing`+`suggested`→`继续构建`；仅 `processing` 且可读→`查看我的信息`（可附提醒）；未知状态／未知 `navTarget`→fail-closed。
...
**帮助落点**：四个承诺与成长路线优先复用现有「说明」帮助弹窗（`help.js`）；用户主动打开后才可见；帮助中**无** PAN-01R 入口。

**PAN-01R（无生产入口）**：PAN-01S Alpha **生产界面必须没有** PAN-01R 入口——含普通 UI、设置、高级、帮助；无隐藏按钮；无可猜测 URL／hash／query；不可经 localStorage／renderer 参数／普通 IPC／production preload 开启。后台模块、安全契约与 hermetic 测试保留。内部 UI 仅显式 test harness（默认 false；主进程测试环境设置；隔离进程；不触碰真实 userData／Package）。新 PAN-04 另行决定协作诊断／用户体验，**不**由 PAN-01S 预留生产入口。

> **废止（historical / superseded）**：「高级／开发者区域包含协作回路验证器」；「Alpha 普通用户界面可以完全没有 PAN-01R 入口」。

**异常**：服从 P0／P1；不伪造所有权／隐私／健康结论；技术细节进设置／高级；不泄漏路径、正文或密钥。

完整实现范围、测试与非目标见 `digitalme_phase1_task_PAN-01S_minimal_product_surface.md`（v0.1.1）。**本规格升版不表示 PAN-01S 已实现。**

**v0.6 / v0.6.1 / v0.6.2 / v0.6.3 变更（PAN-00R + PAN-01S；界面收口由 PAN-01S 实现）**：

- 「产品全貌首页」不再作为「我」的默认表面；PAN-01 密集信息展板裁定为产品感知失败，默认「我」页面由 PAN-01S 收口为极简表面；
- 四个承诺、成长路线与证据链说明迁为**帮助内容**，不作为默认首页主体；
- PAN-01R「体验一次 Digital Me 如何代表我」五步体验撤出普通用户路径；**v0.6.3**：生产设置／高级亦不提供入口，仅内部 test harness；
...
- **v0.6.2**：冻结 §3.1.1 默认「我」细则初稿（部分条件表已被 v0.6.3 取代）；
- **v0.6.3**：冻结 P0→P4 唯一优先级与 PAN-01R 无生产入口；任务包 `codex_review_changes_requested` / `not_started`。
...
#### 7.5.3a 主权协作闭环（PAN-01R · v0.5.2）（**历史 · v0.6／v0.6.3 裁定：仅内部 test harness，无生产入口**）

> **v0.6 裁定（PAN-00R）**：PAN-01R 工程与运行验证通过（`9dd6fa0`；70/70；20/20），但 Owner 产品感知验收未通过。「体验一次 Digital Me 如何代表我」不得继续作为普通用户体验；普通用户入口由 PAN-01S 撤下。底层授权、取消、审计、adopt/reject、推理环境绑定等保留。
>
> **v0.6.3 澄清（PAN-01S Codex 第一轮）**：上述「作为高级/开发者协作回路验证器」口径 **superseded**。PAN-01R 当前**仅**保留为内部测试设施；**不**进入 PAN-01S 生产设置或高级界面；未来新 PAN-04 另行决定协作诊断入口。以下为历史工程规格，保留不删。
...
**当前唯一执行顺序**以 `digitalme_panorama_execution_index_v0.1.md`（v0.2.7）为准。**PAN-01S / PAN-01S.1 / PAN-01S.2 已于 2026-07-20 `accepted`**（Owner real Electron runtime；baseline `cbde807`）。**当前唯一任务**：起草并冻结 PAN-02 独立任务包；PAN-02 = `planned` / `blocked`；renderer foundation R0 = `planned` / `not_started`。不得自行开始 PAN-02～PAN-06 实现。
...
| 2026-07-20 | v0.6.3 状态注记 | PAN-01S / S.1 / S.2 Owner 真机验收 `accepted`（baseline `cbde807`）；更新文首与 §8 当前唯一任务口径；规格正文不升版 |
