...
**能力基线（强制，2026-07-23）**：Digital Me 不得把“像我”实现为通用能力收缩。所接入模型的通用能力必须完整保留；Digital Me 的能力上限和下限均以所接入模型为基线。个人蒸馏结果只能作为主体性叠加层：

- **生成前叠加**：将用户的价值观、经验、判断框架、风格和边界加入任务编排，使模型在完整能力上沿本人方向工作；
- **生成后校对**：先使用模型完整能力形成结果，再按本人价值观、经验、风格、事实和边界校对，违背时修订、标注或请求确认。

如果加入 Digital Me 后通用任务结果明显弱于原始模型，必须按产品缺陷处理，优先检查主体信息注入、相关性门、提示词编排、检索、复核和工具链，不得以“个性化代价”作为默认解释。
...
- **v0.6.4（2026-07-23，能力基线纠正）**：明确“像我”不得削弱大模型通用能力；能力上限/下限以接入模型为基线；蒸馏结果采用生成前叠加或生成后校对；加入后表现变弱视为设计缺陷，不视为个性化必然代价。
