...
> **当前产品主线（2026-07-21，R2 任务包起草）**：**P1-PANORAMA**。**PAN-01S 族 `accepted`**（baseline `cbde807`）。**R0** = **`accepted`**（v0.1.2）。**R1** = **`accepted`**（任务包 **v0.1.3**；baseline **`8d7e9b3`**）。**R2** = **`specified` / `codex_review_pending` / `not_started`**（任务包 **v0.1-draft**；实现分支**不存在**）。**当前唯一等待项：Codex 复核 R2 任务包**；复核通过前不得创建 R2 实现分支或改源码。R2.5 = `planned` / `deferred`。PAN-02 = `planned` / `blocked`。生产默认仍为 **legacy**。
...
**近期工程焦点（2026-07-21 覆盖）**：规格 **v0.6.3** + 决策 **#67～#88**——**R0 / R1 `accepted`**；**R2 v0.1-draft = `specified` / `codex_review_pending` / `not_started`**。**当前唯一等待项：Codex 复核 R2 任务包。** R2.5 = `planned` / `deferred`；**PAN-02** = `planned` / `blocked`。生产默认 **legacy**。
## 4. 系统架构共识（草案）
...
> **2026-07-21 R2 任务包起草覆盖（当前）**：**R0 / R1** = `accepted`。**R2** = **`specified` / `codex_review_pending` / `not_started`**（**v0.1-draft**；实现分支不存在）。**当前唯一等待项：Codex 复核 R2 任务包**；复核通过前不得实现。R2.5 = `planned` / `deferred`。PAN-02 = `planned` / `blocked`。生产默认 **legacy**。
...
88. **Renderer Foundation R2 对话迁移任务包起草（2026-07-21）**：新增 `digitalme_renderer_foundation_R2_chat_and_sessions_migration.md`（**v0.1-draft**）。状态：`specified` / `codex_review_pending` / `not_started`。范围：renderer-next 会话列表与聊天；JSON sessions；类型化 session/chat API；main `activeRequest` 单飞；display/model/attachment 分离；关联文稿紧凑卡；Playwright E2E；整窗回 legacy。实现分支**不存在**。**Codex 复核通过前不得创建实现分支或修改源码。** R1 / R0 / PAN-01S 族 `accepted` 不变；R2.5 = `planned` / `deferred`；PAN-02 = `planned` / `blocked`；生产默认 legacy。
