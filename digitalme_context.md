...
> **界面与功能需求源**：自 2026-07-10 起，桌面应用「用户看见什么、能做什么、何为做完」以 [`digitalme_product_spec_v0.2.md`](digitalme_product_spec_v0.2.md) 为唯一规格；战略与逻辑架构仍以本文为准；**部署与系统拓扑**以 [`digitalme_architecture_edge_sovereign_v0.1.md`](digitalme_architecture_edge_sovereign_v0.1.md) 为准。开发实行**规格驱动**（无规格不排期）。
>
...
### 2.0.1 四板块推进共识（2026-07-13）

在双线指导思想之下，系统推进按四个板块组织（对应建设顺序，非四个并列产品）：

| 板块 | 对应主线 | 核心命题 | 近期取舍 |
|------|----------|----------|----------|
| **① 数字之我构建** | A | 架构精修；采集完整、便捷、**三低成本**；准确性与一致性 | **当前主战场**：养我体验、蒸馏质量、投递箱/分流 |
| **② 能力构建（眼耳口鼻手脚）** | B 的执行面 | 编程、自媒体、营销、写作、商务、运维、重复任务等 | **跟随策略**：不争最强最新，直接导入业界最好能力 |
| **③ 协作雏形** | B 的协作面 | 唯一标识、结算/能力/数据 API；对外展示与匹配；可雇佣/可授权 | **架构已备、运行时分期**：先可被发现与授权，后出租/接活 |
| **④ 快速启动（增长）** | 双线获客 | 关系导入、示范包、开放信息预生成等 | **只做服从主权的冷启动**；不做以匹配/恋爱为主产品的增长捷径 |

正式细则见 §2.6（构建质量）、§7.11（能力跟随）、§7.12（协作与服务面）、§7.13（冷启动与暂缓项）；决策 #36–#39。
...
**主线（当前与长期）**：从人的各类可记录数据中**蒸馏提取综合特征**（表达风格、价值立场、判断框架、记忆、偏好、行为模式等），是**本我复现的关键路径**。当前以文本素材（著作、文章、方案、对话转写等）为主战场，这条主线须持续完善——包括格式适配、蒸馏算法、多源交叉验证、核心层整合与检索增强。
...
### 2.7 审计后第一阶段：主体可信化与协作感知（2026-07-16）
...
第一阶段不做公开市场、自动匹配/接单、自动支付、完整 A2A/AP2/DID 运行时、Web/手机端和新增一级场景。详细执行、角色分工与 DoD 以 `digitalme_phase1_subject_upgrade_plan_v0.1.md` 为唯一执行计划。
...
| 文档 | 作用 |
|---|---|
| `digitalme_product_spec_v0.2.md` | **界面与功能唯一需求源**：IA、工作台、产物、蒸馏/Package/能力/审计、DoD、准入规则 |
| `digitalme_architecture_edge_sovereign_v0.1.md` | **部署与系统拓扑唯一详述**：端主权 × 云边平台；多端职责；云模块优先级；百万 DAU 分期；安全基线 |
| `digitalme_phase1_subject_upgrade_plan_v0.1.md` | **当前第一阶段唯一执行计划**：主体可信化、能力硬化、协作感知、工作包、DoD 与协作机制 |
| `digitalme_architecture_audit_20260716.md` | **审计风险基线**：代码事实、P0/P1/P2 发现、生产就绪判断与整改依据 |
| 本文 | 战略（含 §2.0 双线指导思想）、逻辑架构（§4）、主权、协议、主体层×能力层、决策索引 |
...
**近期工程焦点（2026-07-16 覆盖）**：规格 **v0.4** + 决策 **#57**——主体可信化、主体产品感知、已有能力硬化、外部协作最小骨架；停止扩展新的一级能力面。
...
> **2026-07-16 审计后重排**：第一阶段不再扩展能力面，切换为“主体可信化与协作感知”。`digitalme_phase1_subject_upgrade_plan_v0.1.md` 是当前唯一执行清单；原 v0.3.13 的 L0/审计/CLI 只能视为原型，不视为已达到安全可用。
...
57. **正式架构审计后收敛（2026-07-16）**：架构方向有条件通过，生产就绪不通过；当前为单机 Alpha。第一阶段改为“主体可信化与协作感知”：PackageStore / SecretStore / PolicyEngine / ToolBroker / AuditService / EvalHarness 六内核，主体首页与信任中心，写作/研究/受控执行真实验收，以及默认私有的能力名片、Agent Card 草案、一次性 Interaction Contract 与本地协作模拟。规划+Owner 执行+Cursor/Codex 单任务实现的机制固定化。详见 §2.7、§5.4.1、`digitalme_phase1_subject_upgrade_plan_v0.1.md`。
