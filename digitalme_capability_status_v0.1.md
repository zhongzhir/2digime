...
| **P1-PANORAMA** | `active` / `three_part_alpha_reframed` / `PAN-01S_family_accepted` | 总任务包 v0.4；执行索引 v0.2.8；PAN-00R `accepted`；**PAN-01S / S.1 / S.2 `accepted`（baseline `cbde807`）** | 当前产品主线；**当前唯一任务：起草并冻结 Renderer Foundation R0 独立决策/任务包**（R0 = `planned` / `not_started`，未授权实现）；**PAN-02** = `planned` / `blocked` |
...
| **renderer foundation R0** | `planned` / `not_started` | — | **当前唯一任务范围：仅起草并冻结独立决策/任务包**；未授权实现或创建分支；处理已登记技术债并建立后续开发边界；不重开 PAN-01S |
| **PAN-02** | `planned` / **`blocked`** | — | 待 R0 边界决策后由 Owner/Codex 另行决定；任务包与实现均不得在 R0 决策完成前自行启动 |
