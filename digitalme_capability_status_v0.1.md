...
| **P1-PANORAMA** | `active` / `three_part_alpha_reframed` / `PAN-01S_family_accepted` / `R0_decision_accepted` | 总任务包 v0.4；执行索引 v0.2.12；R0 v0.1.2 **accepted**；R1 任务包 v0.1-draft | **当前唯一任务：Codex 复核 R1**（`codex_review_pending` / `not_started`；无实现分支）；**PAN-02** = `planned` / `blocked` |
...
| **renderer foundation R1** | **`specified` / `frozen_for_implementation` / `codex_review_pending` / `not_started`**；branch = 不存在 | `digitalme_renderer_foundation_R1_shell_and_entry_switch.md` v0.1-draft | 最小 shell + 整窗入口 + 自动回退 + Playwright；**Codex 复核通过前不得实现** |
