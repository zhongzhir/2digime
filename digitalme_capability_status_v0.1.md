...
| **P1-PANORAMA** | `active` / `three_part_alpha_reframed` / `PAN-01S_family_accepted` / `R0_decision_accepted` / `R1_spike_codex_changes_requested` | 总任务包 v0.4；执行索引 v0.2.16；R0 v0.1.2 **accepted**；R1 任务包 **v0.1.3** | **当前唯一等待项：Codex 再复核 R1 有界修复**；正式 shell 不得扩展；**PAN-02** = `planned` / `blocked` |
...
| **renderer foundation R1** | **`implemented` / `spike_partial_verified` / `codex_changes_requested`**（**v0.1.3**）；branch = `codex/r1-renderer-next-shell` | `digitalme_renderer_foundation_R1_shell_and_entry_switch.md` v0.1.3 | spike 主体成立；有界修复待再复核；不得标 empirically_verified/statically_verified/runtime_verified/accepted/released |
