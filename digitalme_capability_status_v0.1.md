...
| Life Graph | `statically_verified`（P1-07 主写回切片） | `src/life/package-write.js`；`builder:previewWrite`/`builder:write` identity；`npm run test:p1-07` 38/38；`test:p1-07-owner-runtime` 8/8；审阅状态与 Package commit 绑定 | **仅** Builder 材料链路的 Life/identity 确认写回已迁 PackageStore；**认知页零散编辑、Policies、MCP、协作仍未迁移**；等待 Codex 再复核与 Owner 临时资料验收，不标 accepted |
