...
| **P1-PANORAMA** | `active` / `three_part_alpha_reframed` / `PAN-01S_family_accepted` / `R0_decision_accepted` | 总任务包 v0.4；执行索引 v0.2.13；R0 v0.1.2 **accepted**；R1 任务包 v0.1.1-draft | **当前唯一任务：Codex 再复核 R1**（`codex_changes_requested` / `not_started`；**未授权实现**）；**PAN-02** = `planned` / `blocked` |
...
| **renderer foundation R1** | **`specified` / `codex_changes_requested` / `not_started`**；branch = 不存在；**未授权实现** | `digitalme_renderer_foundation_R1_shell_and_entry_switch.md` v0.1.1-draft | 入口权限/失败 latch/握手世代/Electron 安全已补冻；**不是** `frozen_for_implementation`；Codex 再复核前不得实现 |
