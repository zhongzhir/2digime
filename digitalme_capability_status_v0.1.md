...
| **P1-PANORAMA** | `active` / `three_part_alpha_reframed` / `PAN-01S_family_accepted` / `R0_decision_accepted` / `R1_accepted` | 总任务包 v0.4；执行索引 v0.2.17；R0 v0.1.2 **accepted**；R1 任务包 **v0.1.3 `accepted`** | **当前阶段：R1 已收口。** **当前唯一下一任务：起草并冻结 R2 独立任务包**（R2 = `planned` / `not_started`；未授权不得实现）；**PAN-02** = `planned` / `blocked` |
...
| **renderer foundation R2** | `planned` / **`not_started`**；branch = 不存在 | — | 下一任务：起草独立任务包；未授权不得创建实现分支或改源码 |
