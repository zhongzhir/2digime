...
| **P1-PANORAMA** | `active` / `three_part_alpha_reframed` / `PAN-01S_family_accepted` / `R0_decision_accepted` | 总任务包 v0.4；执行索引 v0.2.11；PAN-00R `accepted`；**PAN-01S / S.1 / S.2 `accepted`（baseline `cbde807`）**；R0 决策 v0.1.2 **accepted** | 当前产品主线；**当前唯一任务：起草并冻结 R1 独立实施任务包（Codex 复核通过前不得创建实现分支或改源码）**；**PAN-02** = `planned` / `blocked` |
...
| **renderer foundation R1** | 任务包起草中 → 随后 `specified` / `codex_review_pending` / `not_started` | 待 `digitalme_renderer_foundation_R1_shell_and_entry_switch.md` | Codex 复核通过前不得创建实现分支或改源码 |
