...
| **P1-PANORAMA** | `active` / `three_part_alpha_reframed` / `PAN-01S_family_accepted` / `R0_decision_accepted` / `R1_accepted` / `R2_spec_pending_review` | 总任务包 v0.4；执行索引 v0.2.18；R0 v0.1.2 **accepted**；R1 **v0.1.3 `accepted`**；R2 任务包 **v0.1-draft** | **当前唯一等待项：Codex 复核 R2 任务包**；复核通过前不得实现；**PAN-02** = `planned` / `blocked` |
...
| **renderer foundation R2** | **`specified` / `codex_review_pending` / `not_started`**（**v0.1-draft**）；branch = **不存在** | `digitalme_renderer_foundation_R2_chat_and_sessions_migration.md` v0.1-draft | 会话列表+聊天；JSON sessions；类型化 API；main 单飞；**Codex 复核前不得实现** |
