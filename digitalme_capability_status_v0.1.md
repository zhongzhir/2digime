...
| External CLI | `statically_verified` | `src/tool-broker/*`；`external-agent-flow.js`；`npm run test:p1-05` 17/17 + P1-01～04 回归 | 仅 `local_cli_nodejs_v1` 代码所有身份契约 + Authenticode；`shell:false`；**不是 OS 沙箱**；待第四轮 Codex 复核；不安排 Owner 验收 |
...
| ToolBroker (P1-05) | `statically_verified` | `src/tool-broker/*`（profiles/pe-identity/authenticode）；`delegate-runtime`；`npm run test:p1-05` 17/17；P1-01～04 回归；第三轮 Codex 复核修订已合入 | 代码所有 Node.js 身份契约 + Authenticode；禁止 generic PE passthrough；主进程 operationId；sender 绑定停止；orphanRisk 与退出二次确认；待第四轮 Codex 复核；不安排 Owner 验收 |
