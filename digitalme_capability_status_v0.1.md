...
| External CLI | `statically_verified` | `src/tool-broker/*`；`external-agent-flow.js`；`npm run test:p1-05` 14/14 + P1-01～04 回归 | 仅注册工具 `local_cli`（禁止 shell 宿主）；`shell:false`、最小环境、授权目录、超时/取消/输出上限；**不是 OS 沙箱**；待第二轮 Codex 复核与 Owner 沙盒验收 |
...
| ToolBroker (P1-05) | `statically_verified` | `src/tool-broker/*`；`npm run test:p1-05` 14/14；P1-01～04 回归；Package 基线不变；第一轮 Codex 复核修订已合入 | 禁止 shell/脚本宿主；固定 `{{task}}` 契约；可信 taskkill + orphanRisk；退出/销毁 abort；流式完整摘要与截断前缀分离；待第二轮 Codex 复核与 Owner 沙盒验收 |
