...
| **P1-PANORAMA** | `active` / `three_part_alpha_reframed` / `PAN-01S_family_accepted` / `R0_decision_accepted` / `R1_spike_pending_codex_review` | 总任务包 v0.4；执行索引 v0.2.15；R0 v0.1.2 **accepted**；R1 任务包 **v0.1.2** | **当前唯一等待项：Codex 复核 R1 spike**；正式 shell 待定；**PAN-02** = `planned` / `blocked` |
...
| **renderer foundation R1** | **`implemented` / `empirically_verified` / `codex_review_pending`**（**v0.1.2**）；branch = `codex/r1-renderer-next-shell` | `digitalme_renderer_foundation_R1_shell_and_entry_switch.md` v0.1.2 | 兼容性 spike 已完成；版本表已锁；正式 shell 待 Codex；不得标 statically_verified/runtime_verified/accepted/released |
