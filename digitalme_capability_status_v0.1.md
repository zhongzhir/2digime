...
| **P1-PANORAMA** | `active` / `three_part_alpha_reframed` / `PAN-01S_family_accepted` | 总任务包 v0.4；执行索引 v0.2.7；PAN-00R `accepted`；**PAN-01S / S.1 / S.2 `accepted`（baseline `cbde807`）** | 当前产品主线；**下一门槛：PAN-02 任务包（`planned` / `blocked`）；R0 `planned` / `not_started`** |
...
| **PAN-02** | `planned` / **`blocked`** | — | 待独立任务包与启动授权；不得自行开始 |
| **renderer foundation R0** | `planned` / `not_started` | — | 技术债 backlog（displayText 限制、菜单底部定位、并发模型、renderer 单体、旧 prompt/confirm、E2E 不足）；不重新打开 PAN-01S |
