...
| External CLI | `statically_verified` | `src/tool-broker/*`；`external-agent-flow.js`；`npm run test:p1-05` 18/18 + P1-01～04 回归 | 仅 `local_cli_nodejs_v1` 代码所有身份契约 + Authenticode；spawn 前 TOCTOU 重校验；`shell:false`；**不是 OS 沙箱**；待再复核；不安排 Owner 验收 |
...
| ToolBroker (P1-05) | `statically_verified` | `src/tool-broker/*`（profiles/pe-identity/authenticode + spawn 前重校验）；`delegate-runtime`；`npm run test:p1-05` 18/18；P1-01～04 回归；第四轮 TOCTOU 修订已合入 | 代码所有 Node.js 身份契约 + Authenticode；`executable_changed_before_spawn`；主进程 operationId；sender 绑定停止；orphanRisk 与退出二次确认；待再复核；不安排 Owner 验收；不标记 accepted |
