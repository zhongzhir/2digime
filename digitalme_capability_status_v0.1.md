...
| External CLI | `statically_verified` | `src/tool-broker/*`；`external-agent-flow.js`；`npm run test:p1-05` 17/17 + P1-01～04 回归 | 仅 `local_cli` 正向 profile；PE 身份校验；`shell:false`；**不是 OS 沙箱**；待第三轮 Codex 复核与 Owner 沙盒验收 |
...
| ToolBroker (P1-05) | `statically_verified` | `src/tool-broker/*`（含 profiles/pe-identity）；`delegate-runtime`；`npm run test:p1-05` 17/17；P1-01～04 回归；第二轮 Codex 复核修订已合入 | 正向 Tool Profile + PE 身份；主进程 operationId；sender 绑定停止；orphanRisk 提示与退出二次确认；待第三轮 Codex 复核与 Owner 沙盒验收 |
