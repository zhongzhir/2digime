...
| Life Graph | `statically_verified`（P1-07 主写回切片） | `src/life/package-write.js`；`builder:previewWrite`/`builder:write` identity；`npm run test:p1-07` 38/38；任务包 P1-07；Codex R1/R2 硬化（字段确认 / 结构 fail-closed / 来源 id 主进程生成） | **仅** Builder 材料链路的 Life/identity 确认写回已迁 PackageStore；**认知页零散编辑、Policies、MCP、协作仍未迁移**；等待 Codex 再复核与 Owner 临时资料验收，不标 accepted |
