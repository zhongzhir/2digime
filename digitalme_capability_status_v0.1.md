...
| **P1-PANORAMA** | `active` / `three_part_alpha_reframed` / `PAN-01S_family_accepted` / `R0_decision_accepted` / `R1_accepted` / `R2_spec_frozen` | 总任务包 v0.4；执行索引 v0.2.22；R0/R1 **accepted**；R2 **v0.1.1**（规格冻结；未实现） | **当前唯一等待项：Owner 是否授权创建 R2 实现分支**；**PAN-02** = `planned` / `blocked` |
...
| **renderer foundation R2** | **`specified` / `codex_review_passed` / `frozen_for_implementation` / `not_started`**（**v0.1.1**）；branch = **不存在** | `digitalme_renderer_foundation_R2_chat_and_sessions_migration.md` v0.1.1 | 实施规格已冻结；七项合同/22/24/38 保持；三项参数门挡 R2-A；**未获 Owner 授权前不得创建分支或实现** |
