...
| External CLI | `statically_verified` | `src/tool-broker/*`；`external-agent-flow.js`；`npm run test:p1-05` 14/14 + P1-01～04 回归 | 仅注册工具 `local_cli`；`shell:false`、最小环境、授权目录、超时/取消/输出上限；**不是 OS 沙箱**；待 Codex 复核与 Owner 沙盒验收 |
...
| ToolBroker (P1-05) | `statically_verified` | `src/tool-broker/*`；`npm run test:p1-05` 14/14；P1-01～04 回归；Package 基线不变 | 版本化注册表；不可变执行计划；真实 child_process 故障路径；未宣称 OS 沙箱；待 Codex 安全复核与 Owner 沙盒验收 |
