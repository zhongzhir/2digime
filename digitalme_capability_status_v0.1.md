...
| **P1-PANORAMA** | `active` / `three_part_alpha_reframed` / `PAN-01S_pending_spec` | 总任务包 v0.4；执行索引 v0.2；PAN-00R `accepted` | 当前产品主线（三位一体 Alpha）；非代码能力项；**当前唯一任务：起草并冻结 PAN-01S 独立任务包** |
| **PAN-00** | `accepted` | 验收提交 `bc85a14`；Codex 最终复核通过 | 战略与规格冻结完成（历史，不改写） |
| **PAN-00R** | `accepted` | 分支 `codex/pan-00r-three-part-alpha-reset`；任务包 v0.1.2；规格 v0.6.1；证据 `07b631d`（战略修订）+ `6ae2dca`（最小文档修复）；Codex 最终复核通过；Owner 确认 | **docs/strategy acceptance**：三位一体重构与极简产品原则冻结；**不是**运行能力 `released`；不代表 PAN-01S～PAN-06 已实现 |
