...
| **第一纵向闭环** | **`spec_frozen`**；第 1–2 块 **`accepted`**；第 3 块 **`implemented_pending_codex_review`** | 计划 v0.1.6；规格 v0.1.0；`test:vl1-block1/2/3` | 四栏证据 + 有来源约束成果（invocation 须匹配 goal+context version）；**下一项（待第 3 块 accepted）** = 第 4 块 Experience Proposal |
