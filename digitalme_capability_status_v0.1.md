...
| Life Graph | `statically_verified`（P1-07 主写回切片） | `src/life/package-write.js`；`builder:previewWrite`/`builder:write` identity；`npm run test:p1-07` 39/39；`test:p1-07-owner-runtime` 13/13；审阅取消/队列/revision fail-closed | **仅** Builder 材料链路的 Life/identity 确认写回已迁 PackageStore；**认知页零散编辑、Policies、MCP、协作仍未迁移**；等待 Codex 再复核与 Owner 运行验收，不标 accepted |
