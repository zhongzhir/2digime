...
| **P1-PANORAMA** | `active` / `three_part_alpha_reframed` / `PAN-01S_family_accepted` / `R0_decision_accepted` / `R1_spec_frozen` | 总任务包 v0.4；执行索引 v0.2.14；R0 v0.1.2 **accepted**；R1 任务包 **v0.1.1** | **当前唯一等待项：Owner 授权 R1 实现分支与 spike**；R1 implementation `not_started`；**PAN-02** = `planned` / `blocked`；今日不开始实现 |
...
| **renderer foundation R1** | **`specified` / `codex_review_passed` / `frozen_for_implementation` / `not_started`**（**v0.1.1**）；branch = 不存在；**Owner 实现授权未获得** | `digitalme_renderer_foundation_R1_shell_and_entry_switch.md` v0.1.1 | **实施规格已冻结**（非实现完成）；版本表 TBD；获授权后第一步仅为兼容性 spike；不得标 accepted/implemented/statically_verified/runtime_verified/released |
