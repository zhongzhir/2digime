...
| **P1-PANORAMA** | `active` / `three_part_alpha_reframed` / `PAN-01S_specified` | 总任务包 v0.4；执行索引 v0.2.2；PAN-00R `accepted`；PAN-01S 任务包 v0.1.1 | 当前产品主线（三位一体 Alpha）；非代码能力项；**当前唯一任务：Codex 最终复核 PAN-01S 后创建实现分支并编码** |
...
| **PAN-01S** | `specified` / `owner_approved_for_implementation` / `frozen_for_implementation` / `codex_review_changes_requested` / `not_started` | 任务包 v0.1.1；规格 v0.6.3 §3.1.1；规划基线 `488d733` | 极简产品表面；P0→P4 主操作；PAN-01R 无生产入口；**尚未实现**；`owner_approved_for_implementation` ≠ accepted；实现后最高 `statically_verified`；须 Codex 最终复核后开实现分支 |
