...
| **P1-PANORAMA** | `active` / `three_part_alpha_reframed` / `PAN-01S_family_accepted` | 总任务包 v0.4；执行索引 v0.2.9；PAN-00R `accepted`；**PAN-01S / S.1 / S.2 `accepted`（baseline `cbde807`）**；R0 决策稿 v0.1-draft | 当前产品主线；**当前唯一任务：Codex 复核 Renderer Foundation R0**（`spec_drafted` / `codex_review_pending`；implementation `not_started`）；**PAN-02** = `planned` / `blocked` |
...
| **renderer foundation R0** | **`spec_drafted` / `codex_review_pending`**；implementation = `not_started`；branch = 不存在 | `digitalme_renderer_foundation_R0_decision_and_migration_plan.md` v0.1-draft | 架构决策与迁移规格已起草；**不** accepted；**不得**开始 R1；方案 C / TS+React+Vite / strangler R1～R6；不重开 PAN-01S |
| **PAN-02** | `planned` / **`blocked`** | — | 解锁见 R0 §16（R0 确认 + 至少 R2/R3 Owner 真机等）；任务包与实现均不得自行启动 |
