 ...
 * Frozen product. T1–T8 + C1/C2. Same difficulty as Trial-05 final gate.
 * Do not loosen judging. Do not skip tasks. Do not add C3 as a gate.
 */
"use strict";
...
const OUT = path.resolve(
  process.env.DIGITALME_TRIAL_EVIDENCE_DIR || path.join(ROOT, "build", "evidence", "knowledge-worker-broader-trial-gate-01"),
);
...
async function main() {
  ...
  const FOCUS = new Set(
    String(process.env.DIGITALME_KWGATE_FOCUS || "")
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean),
  );
